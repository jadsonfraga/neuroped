...
export const CATEGORIES = {
  animais: "Animais", frutas: "Frutas", transportes: "Transportes", cores: "Cores", opostos: "Conceitos e opostos", objetos: "Objetos do cotidiano",
} as const;
...
export const ITEMS: Item[] = [
  ...sourceManifest.items.map((item): Item => ({
    ...
    note: ["Confira se esta representação faz parte do repertório da criança. Variantes e onomatopeias devem ser descritas; não equivalem automaticamente a nomeação convencional.",
      ["limao", "pessego", "chapeu", "garrafa", "onibus", "tartaruga"].includes(item.id) ? "Exemplar específico: confirme familiaridade e aceite denominações usuais sem exigir espécie, variedade ou modelo." : ""].filter(Boolean).join(" "),
  })),
...
export function ageInMonths(years: string, months: string): number | null {
  ...
  const y=Number(years), m=Number(months), total=y*12+m;
  return m<=11 && total>=12 && total<=215 ? total : null;
}
...
export function eligibleItems(age: number, mode: Mode): Item[] {
  if(!bandFor(age) || !(mode in MODES)) return [];
...
export function makeTrial(targetId:string, mode:Mode, pool:Item[], choices:number, seed:number, index:number): Trial {
  ...
  if(mode!=="nomeacao" || target.pair){
    ...
    const position=shuffle(Array.from({length:size},(_,i)=>i),seed)[index%size];
    optionIds.splice(position,0,target.id);
  }
  const question = mode==="pareamento" ? "Ache a figura igual ao modelo. O modelo continua visível." : mode==="nomeacao" ? target.pair ? "Compare as duas figuras. Como você descreveria a figura destacada?" : target.category==="cores" ? "Que cor é esta?" : "O que é isto?" : target.category==="opostos" ? `Mostre: ${target.label.toLocaleLowerCase("pt-BR")}.` : `Mostre: ${target.label.toLocaleLowerCase("pt-BR")}.`;
...
export function buildPlan(config: Config): Trial[] {
  if(!bandFor(config.ageMonths) || ![2,3,4].includes(config.choices) || !Number.isInteger(config.count) || config.count<1 || config.count>100 || !Number.isInteger(config.seed) || !(config.mode in MODES)) throw new Error("Configuração inválida.");
  ...
  const selected=allowed.filter(item=>config.selectedIds.includes(item.id));
  if(!selected.length) throw new Error("Selecione figuras para iniciar.");
  ...
  // Round-robin prevents a short session being filled entirely by one category.
  ...
  return targets.slice(0,config.count).map((target,index)=>{
    const pool=selected.filter(item=>target.pair ? item.pair===target.pair : target.category==="cores" || config.distractors==="categoria" ? item.category===target.category : item.art===target.art);
...
export function problems(trial:Trial,draft:Draft,events:StageEvent[]):string[]{
  ...
  if(draft.channel==="toque" && interpretable){
    const touches=events.filter(event=>event.kind==="toque");const last=touches.at(-1)?.itemId;
    if(!last) errors.push("Não houve toque registrado. Use apontar/olhar/fala quando a resposta foi observada pelo aplicador.");
    else if((last===trial.targetId)!==(draft.outcome==="correspondente")) errors.push("A classificação não corresponde ao último toque. Descreva uma resposta por outra via separadamente.");
  }
...
export function reportText(config:Config,plan:Trial[],ledger:Observation[]):string{
  const records=activeObservations(ledger);
  const primary=new Set(records.filter(record=>!record.trial.adaptedFrom).map(record=>record.trial.id));
  ...
  for(const record of records){
    ...
    header.push("",`${record.item.label} · ${CATEGORIES[record.item.category]} · ${MODES[record.trial.mode]}`,`Pergunta: ${record.trial.question}`,record.trial.context?`Contexto lido: ${record.trial.context}`:"",`Ordem mostrada: ${record.options.map(option=>option.label).join(" | ")}`,`Situação: ${r.outcome?OUTCOMES[r.outcome]:"não registrado"}. Via: ${r.channel?CHANNELS[r.channel]:"não informada"}. Ajuda: ${SUPPORTS[r.help]}. Familiaridade: ${r.familiarity}.`,`Resposta literal: ${r.literal || "não verbalizada / não transcrita"}`,`Observações: ${r.note || "sem observação adicional"}`,`Apresentações: ${record.events.filter(event=>event.kind==="apresentado").length}; toques: ${record.events.filter(event=>event.kind==="toque").map(event=>itemFor(event.itemId!).label).join(" → ") || "nenhum"}.`,record.previousExposure?"Houve exposição prévia ao nome/ajuda nesta sessão: não interpretar como nomeação espontânea independente.":"",record.trial.adaptedFrom?"Tentativa complementar: manter separada da oportunidade inicial.":"",record.supersedes?`Registro corrigido com histórico preservado. Motivo: ${record.correctionReason}`:"");
  }
  header.push("","Síntese diagnóstica e assinatura: exclusivas do profissional após integração clínica.","Ilustrações: Mulberry Symbols, © 2018–2026 Steve Lee, CC BY-SA 4.0. Cores e relações geométricas: implementação autoral NeuroPed. Repertório visual ainda não validado psicometricamente em crianças brasileiras.");
