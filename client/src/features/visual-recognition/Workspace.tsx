...
export default function VisualRecognitionWorkspace(){
  ...
  const save=(complement=false)=>{
    ...
    try{
      ...
      setLedger(current=>[...current,record]);setMessage("");setDraft(emptyDraft());setEvents([]);
      if(editing){setEditing(null);setCorrection("");setPhase("report");return;}
      const nextQueue=extra?[...queue.slice(0,index+1),extra,...queue.slice(index+1)]:queue;
      if(extra)setQueue(nextQueue);
      if(index+1>=nextQueue.length)setPhase("report");else setIndex(index+1);
  ...
  return <div className={`rv-workspace ${displayAge>=84?"rv-mature":""}`}>
    ...
    {phase==="prepare"&&<>
      <section className="rv-panel"><h2><SlidersHorizontal size={21}/> 1. Escolha como observar</h2><div className="rv-form-grid">
        ...
        <label>Modalidade<select value={mode} onChange={event=>{setMode(event.target.value as Mode);setExcluded([]);setChecks([false,false,false]);}}>{Object.entries(MODES).map(([id,label])=><option key={id} value={id}>{label}</option>)}</select></label>
        <label>Alternativas por tela<select value={choices} disabled={mode==="nomeacao"} onChange={event=>setChoices(Number(event.target.value) as 2|3|4)}><option value={2}>2 · campo reduzido</option><option value={3}>3 · campo intermediário</option><option value={4}>4 · campo ampliado</option></select></label>
        <label>Tamanho da sessão<select value={count} onChange={event=>setCount(Number(event.target.value))}><option value={6}>Até 6 oportunidades</option><option value={12}>Até 12 oportunidades</option><option value={20}>Até 20 oportunidades</option><option value={100}>Todo o conjunto selecionado</option></select></label>
        <label>Outras figuras na tela<select value={distractors} onChange={event=>setDistractors(event.target.value as Config["distractors"])}><option value="distantes">Categorias distintas quando possível</option><option value="categoria">Mesma categoria</option></select></label>
    ...
    {phase==="run"&&trial&&target&&config&&<section className="rv-panel rv-run">
      <div className="rv-run-top"><span className="rv-eyebrow">{editing?"CORREÇÃO COM HISTÓRICO":trial.adaptedFrom?"TENTATIVA COMPLEMENTAR":`OPORTUNIDADE ${index+1} DE ${queue.length}`}</span><button type="button" onClick={()=>{setEditing(null);setPhase("report");setMessage("");}}>Revisar registros</button></div>
