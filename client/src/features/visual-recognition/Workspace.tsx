...
import { ITEMS, CATEGORIES, MODES, BANDS, VERSION, NATURE, OUTCOMES, SUPPORTS, CHANNELS, itemFor, ageInMonths, bandFor, eligibleItems, buildPlan, makeTrial, emptyDraft, observe, activeObservations, reportText, easyPlanFor, type Category, type Mode, type Config, type Trial, type StageEvent, type Draft, type Observation } from "./model";
...
export default function VisualRecognitionWorkspace(){
  ...
  const startEasy=async()=>{
    ...
    const settings=easyPlanFor(age);
    const next:Config={ageMonths:age,mode:"receptivo",choices:settings.choices,count:settings.count,selectedIds:settings.ids,seed:crypto.getRandomValues(new Uint32Array(1))[0],distractors:settings.distractors,contextAcknowledged:false,conditions:[`Modo Fácil (joguinho): reconhecimento por toque na tela, ${settings.choices} alternativas, sem conferências de preparo`]};
    try{
      const plan=buildPlan(next);setLoading(true);abort.current?.abort();const controller=new AbortController();abort.current=controller;
      const loaded=await preloadSymbols(next.selectedIds,controller.signal);if(controller.signal.aborted){Object.values(loaded).forEach(url=>URL.revokeObjectURL(url));return;}
      Object.values(liveUrls.current).forEach(url=>URL.revokeObjectURL(url));liveUrls.current=loaded;setUrls(loaded);
  ...
  return <div className={`rv-workspace ${displayAge>=84?"rv-mature":""}`}>
    ...
    {easy&&<>
      ...
      <div className="rv-age-note"><strong>{age===null?"Informe a idade exata para montar o jogo":`Idade informada: ${age} meses · ${band?.label}`}</strong><p>{age===null?"Modo Fácil: a idade define quantas figuras e quantas alternativas. A criança toca; o jogo passa sozinho.":`Modo Fácil: ${easyPlanFor(age).count} figuras para reconhecer (“Mostre…”), ${easyPlanFor(age).choices} alternativas por vez. A criança toca; o jogo passa sozinho.`}</p></div>
