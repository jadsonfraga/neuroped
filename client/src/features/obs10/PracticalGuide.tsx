...
import { AGE_BANDS, type AgeBand, type Outcome } from "./protocol";
  ...
  return <section className="obs10-panel obs10-materials-first obs10-no-print" data-testid="obs10-materials-first">
    ...
    <div ref={root} className="obs10-printable-kit">
      ...
      <fieldset disabled={locked} className="obs10-kit-grid">{items.map(({ id, quantity }) => <article className="obs10-material" key={id} data-material={id}>
        ...
        <div><strong>{MATERIALS[id].label}</strong><span className="obs10-quantity">{quantity ?? MATERIALS[id].quantity}</span><p>{MATERIALS[id].detail}</p><details><summary>Substituto seguro</summary><p>{MATERIALS[id].substitute}</p>{id === "book" && <p>Cenas avulsas substituem apenas apontar/descrever. Para observar folhear e virar páginas, mantenha o livro físico.</p>}</details></div>
      ...
      <details className="obs10-scenes"><summary>Cenas simples para imprimir antes da consulta</summary><p>Alternativa ao livro somente para apontar ou descrever figuras. Escolha a cena antes. Mostre apenas o desenho, sem legenda ou resposta. Não são estímulos padronizados. Não usar estas figuras para ensinar respostas de memória. Para folhear, use livro físico.</p><div className="obs10-print-cards"><div><TaskPicture scene="picture-play" label="Cena A para apontar ou descrever" /></div><div><TaskPicture scene="picture-eat" label="Cena B para apontar ou descrever" /></div><div><TaskPicture scene="picture-cat" label="Cena C para apontar ou descrever" /></div></div></details>
    </div>
