...
export default function TabletWorkspace({ onClose }: { onClose: () => void }) {
  ...
  return <div className={`obs10 ot-root ${large ? "ot-large" : ""}`} data-testid="obs10-tablet" data-phase={state.phase}>
    ...
    {state.phase === "review" && state.record && <section className="ot-card">
      ...
      <label className="ot-check"><input type="checkbox" checked={state.record.reviewed} onChange={(e) => dispatch({ type: "reviewed", value: e.target.checked })} />Conferi descrições, ajudas e estações não observadas. Esta é uma declaração minha, não uma assinatura médica.</label>
