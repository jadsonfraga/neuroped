...
export default function TabletWorkspace({ onClose }: { onClose: () => void }) {
  ...
  const transitionTask = transitionTaskId ? plan?.tasks.find((entry) => entry.id === transitionTaskId) : undefined;
  const transitionObservation = transitionTaskId ? state.record?.observations.find((entry) => entry.taskId === transitionTaskId) ?? null : null;
  const betweenStations = state.phase === "cue" && Boolean(transitionTask && transitionObservation && task);
  ...
  return <div className={`obs10 ot-root ${large ? "ot-large" : ""}`} data-testid="obs10-tablet" data-phase={state.phase}>
    ...
    {state.phase === "camera" && <section className="ot-card">
      ...
      <p>A seleção frontal é solicitada ao navegador; confira a câmera realmente escolhida. Nenhuma prévia comprova que rosto e mãos ficarão visíveis durante toda a estação.</p>
    ...
    {state.phase === "rehearsal" && <section className="ot-card">
      <p>Treino do aplicador, sem criança, gravação ou contagem de tempo. A instrução aparece primeiro; depois só a estação; por último, o registro.</p>
    ...
    {state.phase === "ready" && <section className="ot-card">
      <p><strong>{plan?.bandLabel} · {plan?.tasks.length} estações digitais ou de interação.</strong></p><p>Diga ao responsável: “Não é prova nem nota. Fique perto sem dar dicas. Podemos parar a qualquer momento.”</p>
      ...
      {plan?.tasks.some((t) => t.id.endsWith(":movement")) && <p>Uma das estações pede espaço livre para caminhar e o tablet apoiado de pé, mostrando o corpo inteiro. Confira o espaço antes de começar.</p>}
    ...
    {state.phase === "cue" && !betweenStations && task && <section className="ot-card ot-station-card" data-testid="tablet-cue">
    ...
    {state.phase === "response" && task && <section className="ot-card ot-station-card" data-testid="tablet-response">
      ...
      <button type="button" className="ot-primary" disabled={!outcome} onClick={() => { if (outcome) { const hasNext = Boolean(plan && state.cursor + 1 < plan.tasks.length); const closedTaskId = task.id; apply({ type: "save", outcome, note }); if (hasNext) setTransitionTaskId(closedTaskId); setNote(""); setOutcome(""); } }}>{note.trim() ? "Salvar resposta e continuar" : "Marcar categoria e continuar · detalhar depois"}</button><button type="button" onClick={() => end("Encerramento com descrição ainda pendente")}>Encerrar e completar depois</button>
    </section>}
