...
const task = (id: string, title: string, kind: TabletKind, seconds: number, command: string, observe: string, extra: Partial<TabletTask> = {}): TabletTask => ({
  ...
  prepare: kind === "quiet" ? "Tablet voltado para você. Mostre rosto e mãos na câmera, sem retirar apoios habituais." : "Explique antes. Ao tocar em Abrir estação, só o estímulo aparece. A criança pode apontar, falar ou tocar; não precisa dominar o tablet.",
...
const recall = task("recall", "Retome as palavras sem pistas", "quiet", 30, "Quais eram as três palavras que pedi para guardar?", "Registre literalmente a evocação. O intervalo é entre aberturas registradas das estações, não tempo verificado do vídeo nem medida isolada de memória.", { memory: "recall", caution: "Não dar categorias, sílabas, figuras ou outras pistas. Sem aprendizagem inicial registrada, não interpretar retenção." });
