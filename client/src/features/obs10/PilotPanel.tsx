...
import { DIFFICULTY_OPTIONS, REPEAT_OPTIONS, UTILITY_OPTIONS, emptyPilot, pilotMetrics, WORK_PHASES, type PilotRecord, type WorkPhase } from "./pilot";
import { PilotConsolidation } from "./PilotConsolidation";
...
export function PilotPanel({ record, stage, activePhase, seconds, onStart, onStop, onChange }: {
  ...
  function downloadMetrics() {
    ...
    // The file name carries no clinical date: the whitelist export must not be linkable to a consultation time.
    const suffix = typeof crypto.randomUUID === "function" ? crypto.randomUUID().slice(0, 8) : String(Math.floor(Math.random() * 1e8));
    const a = document.createElement("a"); a.href = url; a.download = `OBS10-metricas-operacionais-${suffix}.json`; document.body.append(a); a.click(); a.remove(); setTimeout(() => URL.revokeObjectURL(url), 1000);
  }
  return <details className="obs10-guide obs13-pilot" data-testid="obs13-pilot">
    ...
    {stage === "finished" && <>
      <label>Dificuldade operacional observada<select aria-label="Dificuldade operacional observada" value={p.difficulty} onChange={(ev) => onChange({ ...p, difficulty: ev.target.value as PilotRecord["difficulty"] })}>{DIFFICULTY_OPTIONS.map((s) => <option key={s}>{s}</option>)}</select></label>
      <fieldset disabled={!medical}>
        <legend>Opinião do profissional · não é medida de acurácia</legend>
        <label>Utilidade para a consulta<select aria-label="Utilidade para a consulta" value={p.utility} onChange={(ev) => onChange({ ...p, utility: ev.target.value as PilotRecord["utility"] })}>{UTILITY_OPTIONS.map((s) => <option key={s}>{s}</option>)}</select></label>
        <label>Necessidade de repetir tarefas<select aria-label="Necessidade de repetir tarefas" value={p.repeatNeed} onChange={(ev) => onChange({ ...p, repeatNeed: ev.target.value as PilotRecord["repeatNeed"] })}>{REPEAT_OPTIONS.map((s) => <option key={s}>{s}</option>)}</select></label>
      </fieldset>
    ...
    {stage !== "running" && <PilotConsolidation disabled={Boolean(activePhase)} />}
