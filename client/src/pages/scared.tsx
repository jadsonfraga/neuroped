...
export default function ScaredPage() {
  ...
  const answered = scaredQuestions.reduce((count, _, i) => count + (answers[i] !== undefined ? 1 : 0), 0);
  const progress = (answered / total) * 100;
  const allAnswered = answered === total;
  const firstMissingIndex = scaredQuestions.findIndex((_, i) => answers[i] === undefined);
              ...
              >
                {scaredLabels.map((label, j) => (
                  <div key={j} className="flex items-center">
                    <RadioGroupItem value={j.toString()} id={`scared-q${i}-o${j}`} className="peer sr-only" />
                    <Label
