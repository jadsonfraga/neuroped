  ...
  if (showResult) {
    return (
      <div className="space-y-6">
            ...
            {pantDomains.map((domain) => {
              ...
              return (
                <div key={domain.id} className="space-y-2">
                  <h3
                    className={`text-xs font-semibold flex items-center gap-1.5 ${domainTextColor[domain.id]}`}
                  >
                    <Icon className="w-4 h-4" /> Macrodomínio {domain.id}:{" "}
                    {domain.short}
                  </h3>
                  {dScales.map((scale) => (
                    <div
        ...
        <SaveToPatient
          scaleName="PANT"
          responses={pantScales.map((scale) => ({
            question: scale.name,
            answer:
              answers[scale.number] === undefined
                ? "—"
  ...
  return (
    <>
      <div className="space-y-6 print:hidden">
        ...
        >
          ...
          >
            ...
            <span className="hidden sm:inline">Imprimir p/ família</span>
          </Button>
        </PageHero>
        ...
        <div className="rounded-xl bg-primary/5 border border-primary/20 p-4">
          <p className="text-xs text-foreground/80 leading-relaxed">
            <strong>Instruções:</strong> Para cada escala passiva, atribua o
            nível observado de 0 a 4 conforme a régua de regulação. Use o{" "}
            <strong>exemplo do dia a dia</strong> em cada item para responder
            com mais segurança, avaliando espontaneidade, frequência,
            generalização e impacto funcional no cotidiano.
          </p>
        </div>
      ...
      <div className="hidden print:block text-black text-[11px] leading-snug">
        ...
        <p className="text-xs">
          Avaliação Funcional do Neurodesenvolvimento — para a família observar
          e responder em casa.
        </p>
        <p className="text-[10px] mt-1">
          Em cada item, marque o número de 0 a 4 que melhor descreve o dia a dia
          da criança, usando os exemplos como referência.
        </p>
                ...
                {scales.map((s) => (
                  <li key={s.number} className="break-inside-avoid">
                    <div className="flex justify-between gap-3">
                      <span>
                        <b>{String(s.number).padStart(2, "0")}.</b> {s.name}
                      </span>
                      <span className="whitespace-nowrap font-mono tracking-widest">
                        0 1 2 3 4
                      </span>
                    </div>
                    {pantExamples[s.number] && (
                      <div className="text-[10px] text-black/70">
                        Ex.: {pantExamples[s.number]}
                      </div>
                    )}
                  </li>
                ))}
        ...
        <p className="mt-3 text-[9px] text-black/60">
          Documento de apoio à observação familiar — não substitui avaliação
          clínica. NeuroPed · Dr. Jadson Fraga.
        </p>
      </div>
    </>
  );
