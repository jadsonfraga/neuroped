    ...
    <div className="mx-auto w-full max-w-7xl space-y-5 px-4 py-6 sm:px-6 lg:px-8">
      <div className="flex flex-col gap-4 lg:flex-row lg:items-end lg:justify-between">
        <div>
          ...
          <h1 className="text-3xl font-semibold tracking-tight">Experiências integradas do NeuroPed</h1>
          <p className="mt-2 max-w-3xl text-sm leading-6 text-muted-foreground">A Secretária IA já possui rota pública própria em <strong className="text-foreground">/marcacao</strong> e não é mais duplicada neste hub. Aqui permanecem apenas as experiências complementares que ainda precisam de abertura integrada ou externa.</p>
        </div>
      ...
      <Card className="border-primary/20 bg-primary/[0.03]">
        <CardContent className="flex flex-col gap-3 py-4 text-sm text-muted-foreground sm:flex-row sm:items-start">
          ...
          <p><strong className="text-foreground">Isolamento seguro:</strong> experiências externas continuam no domínio original. O NeuroPed não recebe senhas, não copia dados do Google e não grava informações externas no SQLite clínico.</p>
        </CardContent>
      </Card>

      <div className="flex flex-wrap gap-2 rounded-2xl border bg-card p-2" role="tablist" aria-label="Experiências integradas">
        {sites.map((site) => {
      ...
      <Card id={`manus-panel-${activeSite.id}`} role="tabpanel" aria-label={activeSite.label} className="overflow-hidden">
        <CardHeader className="border-b bg-muted/20">
          <div className="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between">
            <div>
              <div className="mb-2 flex items-center gap-2"><Badge variant="outline">Integração</Badge><span className="text-xs text-muted-foreground">{activeSite.url}</span></div>
          ...
          {(frameStatus === "loading" || frameStatus === "timeout" || frameStatus === "error") && (
            <div className="absolute inset-x-0 top-0 z-10 flex min-h-32 items-center justify-center border-b bg-background/95 p-6 text-center backdrop-blur-sm" role="status">
              <div className="max-w-xl space-y-3">
                <p className="text-sm font-semibold text-foreground">{frameStatus === "loading" ? "Carregando experiência…" : "Esta experiência não pode ser exibida dentro do NeuroPed."}</p>
                <p className="text-xs leading-5 text-muted-foreground">{frameStatus === "loading" ? "Se o conteúdo não aparecer em alguns segundos, abra em uma nova guia." : "O domínio pode bloquear incorporação por política de segurança ou exigir login próprio. Seus dados clínicos continuam isolados."}</p>
