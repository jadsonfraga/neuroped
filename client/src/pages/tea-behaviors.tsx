...
import {
  teaAgeGroups, teaConceptualBase, behaviorRatingLabels, classifyTeaBehaviors,
  type TeaAgeGroup
      ...
      <CardContent className="p-4 space-y-3">
        <button onClick={() => setExpanded(!expanded)} className="flex items-center justify-between w-full text-left">
          <div className="flex items-center gap-2">
            ...
            <h3 className="text-sm font-bold text-blue-800 dark:text-blue-300">Base Conceitual — DSM-5 Critério B</h3>
          </div>
          {expanded ? <ChevronUp className="w-4 h-4 text-blue-600" /> : <ChevronDown className="w-4 h-4 text-blue-600" />}
        </button>
        {expanded && (
          <div className="space-y-3 pt-1">
            <p className="text-xs text-blue-800 dark:text-blue-300">{teaConceptualBase.dsm5Criteria}</p>
            <div className="space-y-1">
              <p className="text-xs font-semibold text-blue-800 dark:text-blue-300">4 Domínios de RRBs:</p>
              {teaConceptualBase.domains.map((d, i) => (
                <div key={i} className="flex items-start gap-2 ml-2">
                  <span className="text-xs text-blue-600 font-bold mt-0.5">{i + 1}.</span>
                  <span className="text-xs text-blue-700 dark:text-blue-300">{d}</span>
                </div>
              ))}
            </div>
            <div className="rounded-lg bg-blue-100 dark:bg-blue-900/30 p-3 space-y-2">
              <p className="text-xs text-blue-800 dark:text-blue-200 leading-relaxed">{teaConceptualBase.developmentalNote}</p>
              <p className="text-xs text-blue-800 dark:text-blue-200 leading-relaxed italic">{teaConceptualBase.functionalNote}</p>
            </div>
            <div className="space-y-1">
              <p className="text-xs font-semibold text-blue-800 dark:text-blue-300">O que torna o achado clinicamente relevante:</p>
              <div className="flex flex-wrap gap-1.5">
                {teaConceptualBase.clinicalRelevance.map((r) => (
                  <Badge key={r} variant="outline" className="text-xs text-blue-700 dark:text-blue-300 border-blue-300">{r}</Badge>
                ))}
              </div>
            </div>
          </div>
...
function AgeGroupSelector({ onSelect }: { onSelect: (g: TeaAgeGroup) => void }) {
  return (
    <div className="space-y-6">
      <div className="flex items-center gap-3">
        ...
        <div>
          ...
          <p className="text-xs text-muted-foreground">100 itens por faixa etária — Síntese clínica baseada no DSM-5</p>
        </div>
      </div>
      ...
      <div className="rounded-xl bg-emerald-50 dark:bg-emerald-950/20 border border-emerald-200 dark:border-emerald-800/40 p-4">
        <div className="flex items-start gap-2">
          ...
          <p className="text-xs text-emerald-800 dark:text-emerald-300 leading-relaxed">
            Selecione a faixa etária do paciente para avaliar a presença e intensidade dos comportamentos repetitivos restritos (RRBs), alterações sensoriais e padrões de rigidez.
          </p>
        </div>
      </div>
          ...
          >
            <CardContent className="p-4 space-y-2">
              <div className="flex items-start justify-between">
                <div className={`w-9 h-9 rounded-xl bg-gradient-to-br ${group.gradient} flex items-center justify-center shadow-sm`}>
              ...
              <div>
                <div className="flex items-center gap-2">
                  ...
                  <Badge className={`text-xs bg-${group.gradient.split("-")[1]}-100 text-${group.gradient.split("-")[1]}-700 dark:bg-${group.gradient.split("-")[1]}-900/40 dark:text-${group.gradient.split("-")[1]}-300`}>
                    {group.ageRange}
                  </Badge>
                </div>
              </div>
      ...
      <Card className="border-primary/30 bg-gradient-to-br from-primary/5 to-chart-2/5 dark:from-primary/10 dark:to-chart-2/10">
        <CardContent className="p-4 space-y-2">
          <div className="flex items-center gap-2">
            ...
            <h3 className="text-sm font-bold text-foreground">Sobre a Escala</h3>
          </div>
          <p className="text-xs text-muted-foreground leading-relaxed">
            Esta escala apresenta 100 comportamentos atípicos organizados por 6 faixas etárias (0-18 anos). Um mesmo comportamento pode surgir em mais de uma faixa. A relevância clínica depende de frequência, intensidade, inflexibilidade, interferência funcional e desproporção para a idade.
          </p>
          <div className="flex flex-wrap gap-1.5 pt-1">
            <Badge variant="outline" className="text-xs">100 Itens</Badge>
            <Badge variant="outline" className="text-xs">6 Faixas Etárias</Badge>
            <Badge variant="outline" className="text-xs">4 Domínios RRB</Badge>
            <Badge variant="outline" className="text-xs">DSM-5 Critério B</Badge>
          </div>
        </CardContent>
      </Card>
    </div>
...
function AgeGroupEvaluation({ group, onBack }: { group: TeaAgeGroup; onBack: () => void }) {
  ...
  if (showResult) {
    ...
    return (
      <div className="space-y-6">
        <div className="flex items-center gap-3">
          <div className={`w-10 h-10 rounded-xl bg-gradient-to-br ${group.gradient} flex items-center justify-center shadow-sm`}>
            <Fingerprint className="w-5 h-5 text-white" />
          </div>
          <div>
            ...
            <div className="rounded-xl bg-amber-50 dark:bg-amber-950/20 border border-amber-200 dark:border-amber-800/40 p-4">
              <div className="flex items-start gap-2">
                ...
                <div className="text-xs text-amber-800 dark:text-amber-300 space-y-1">
                  <p><strong>Nota:</strong> Esta escala é uma ferramenta de rastreio educacional. O diagnóstico de TEA requer avaliação multidisciplinar completa, incluindo observação clínica, entrevista e histórico desenvolvimental.</p>
                  <p>Nem todo item aparece em toda criança. Um mesmo comportamento pode surgir em mais de uma faixa etária.</p>
                </div>
              </div>
            </div>
          </CardContent>
    ...
    <div className="space-y-6">
      ...
      <div className="flex items-center gap-3">
        ...
        <div>
          ...
          <p className="text-xs text-muted-foreground">{group.ageRange} — {group.items.length} comportamentos</p>
        </div>
      </div>

      <div className="space-y-2">
        <div className="flex justify-between text-xs text-muted-foreground">
          <span>{answered} de {total} avaliados</span>
      ...
      <div className="rounded-xl bg-emerald-50 dark:bg-emerald-950/20 border border-emerald-200 dark:border-emerald-800/40 p-4">
        <p className="text-xs text-emerald-800 dark:text-emerald-300 leading-relaxed">
          <strong>Instruções:</strong> Para cada comportamento, avalie a frequência/intensidade com que é observado no paciente. Considere o padrão habitual, não episódios isolados.
        </p>
      </div>
      ...
      {group.items.map((item) => (
        <Card key={item.id} className="border-card-border">
          <CardContent className="p-4 space-y-3">
            <div className="flex items-start gap-2">
              <Badge variant="outline" className="text-xs font-mono flex-shrink-0 mt-0.5">{item.id}</Badge>
            ...
            >
              {behaviorRatingLabels.map((label, j) => (
                <div key={j} className="flex items-center">
                  <RadioGroupItem value={j.toString()} id={`tea-b-${item.id}-o${j}`} className="sr-only" />
                  <Label
                    htmlFor={`tea-b-${item.id}-o${j}`}
                    className={`text-xs px-3 py-1.5 rounded-full border cursor-pointer transition-colors ${
                      answers[item.id] === j
                        ? j === 0 ? "bg-emerald-500 text-white border-emerald-500"
                          : j === 1 ? "bg-amber-500 text-white border-amber-500"
                          : j === 2 ? "bg-orange-500 text-white border-orange-500"
      ...
      >
        {answered >= total ? "Ver Resultado" : `Avalie todos os ${total} comportamentos`}
      </Button>
    </div>
  );
}
...
export default function TeaBehaviorsPage() {
  ...
  if (selectedGroup) {
    return <AgeGroupEvaluation group={selectedGroup} onBack={() => setSelectedGroup(null)} />;
  }
