...
export default function SondaDezDailyPage() {
  ...
  return (
    <>
      <section className="mx-auto mb-5 w-full max-w-6xl overflow-hidden rounded-[30px] border border-primary/15 bg-gradient-to-br from-primary/[0.08] via-background to-cyan-50/70 shadow-sm dark:to-cyan-950/10">
          ...
          <div className="flex flex-col gap-4 lg:flex-row lg:items-start lg:justify-between">
            <div className="min-w-0">
              ...
              <p className="mt-2 max-w-4xl text-sm leading-relaxed text-muted-foreground">
                O fluxo diário agora explicita três camadas: preparação segura, especialização por idade e grau de mediação. O núcleo observacional, os tempos, o banco visual e as travas clínicas permanecem preservados.
              </p>
            </div>
            <Button type="button" variant="outline" className="shrink-0 rounded-xl" onClick={() => setExpanded((value) => !value)}>
              {expanded ? <ChevronUp className="mr-2 h-4 w-4" /> : <ChevronDown className="mr-2 h-4 w-4" />}
              {expanded ? "Recolher guia" : "Abrir guia"}
            </Button>
          </div>
