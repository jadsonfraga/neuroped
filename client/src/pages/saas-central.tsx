      ...
      case "reminders":
        return (
          <ActionCard title="Régua de lembretes" description="A régua consulta a agenda canônica e respeita consentimento, fuso e horário silencioso.">
            <div className="space-y-3">{["48 horas antes", "24 horas antes", "Recuperação de falta"].map((item, index) => <label key={item} className="flex items-center justify-between rounded-xl border p-3"><span className="text-sm font-medium">{item}</span><input type="checkbox" defaultChecked={index < 2} className="h-4 w-4 accent-primary" /></label>)}</div><Button className="mt-4 gap-2" onClick={() => recordAction("Régua simulada sem envio externo.", 2)}><Bell className="h-4 w-4" />Simular régua</Button>
          </ActionCard>
        );
      case "intake":
        return (
