...
import { softTick } from "@/lib/softSounds";
...
const PHYSICAL: BallardItem[] = [
  {
    name: "Pele",
    grades: {
      [-1]: "Pegajosa, friável, transparente",
      0: "Gelatinosa, vermelha, translúcida",
      1: "Lisa, rosada, veias visíveis",
      2: "Descamação superficial e/ou rash, poucas veias",
      3: "Rachaduras, áreas pálidas, veias raras",
      4: "Tipo pergaminho, rachaduras profundas, sem vasos",
      5: "Tipo couro, rachada, enrugada",
    },
  },
  {
    name: "Lanugo",
    grades: {
      ...
      2: "Afinando",
      3: "Áreas calvas",
      4: "Predominantemente calvo",
    },
  },
  {
    name: "Superfície plantar",
    grades: {
      [-1]: "Calcanhar-artelho 40–50 mm (−1) / <40 mm (−2)",
      0: ">50 mm, sem prega",
      1: "Marcas vermelhas discretas",
      2: "Apenas prega transversal anterior",
      3: "Pregas nos 2/3 anteriores",
      4: "Pregas em toda a sola",
    },
  },
  {
    name: "Mama",
    grades: {
      [-1]: "Imperceptível",
      0: "Quase imperceptível",
      1: "Aréola plana, sem broto",
      2: "Aréola pontilhada, broto de 1–2 mm",
      3: "Aréola elevada, broto de 3–4 mm",
      4: "Aréola completa, broto de 5–10 mm",
    },
  },
  {
    name: "Olho/Orelha",
    grades: {
      [-1]: "Pálpebras fundidas: frouxamente (−1) / firmemente (−2)",
      0: "Pálpebras abertas, pavilhão plano, permanece dobrado",
      1: "Pavilhão levemente curvo, mole, recuo lento",
      2: "Pavilhão bem curvo, mole mas com recuo pronto",
      3: "Formado e firme, recuo instantâneo",
      4: "Cartilagem espessa, orelha rígida",
    },
  },
  {
    name: "Genitais (masc./fem.)",
    grades: {
...
export default function BallardPage() {
  ...
  const all = useMemo(
    () => [
      ...NEUROMUSCULAR.map((it) => ({ ...it, g: "nm" })),
      ...PHYSICAL.map((it) => ({ ...it, g: "ph" })),
    ],
    [],
  );
  const answered = Object.keys(scores).length;
  const complete = answered === all.length;
  const reportItems = all.map((item, index) => {
    const key = `${item.g}-${index % 6}`;
    const selected = scores[key];
    const descriptor = item.grades?.[selected];
    return {
      question: `[${item.g === "nm" ? "Maturidade Neuromuscular" : "Maturidade Física"}] ${item.name}`,
      answer:
        selected === undefined
          ? "Não respondida"
          : descriptor
            ...
            <CardContent className="p-4 space-y-2">
              ...
              {!it.grades && (
                <p className="text-[11px] text-muted-foreground">
                  Item postural — atribua o grau conforme o diagrama do
                  escoresheet à beira-leito.
                </p>
              )}
    ...
    <div className="space-y-6">
      <div className="flex items-center gap-3">
        ...
        <div>
          ...
          <p className="text-xs text-muted-foreground italic">
            Estimativa da idade gestacional por maturidade — recém-nascido
          </p>
        </div>
      </div>
      ...
      <div className="rounded-xl bg-blue-50 dark:bg-blue-950/20 border border-blue-200 dark:border-blue-800/40 p-4">
        <p className="text-xs text-blue-800 dark:text-blue-300 leading-relaxed">
          <strong>Instruções:</strong> pontue cada item de −1 a 5. Some os 12
          itens (maturidade neuromuscular + física); o escore total é convertido
          em idade gestacional estimada (−10 → 20 semanas … 0 → 24 … 50 → 44).
          Aplicar idealmente nas primeiras 12–24 h de vida.
        </p>
      </div>
      ...
      <div className="sticky top-0 z-20 -mx-1 rounded-xl border bg-background/95 p-3 shadow-sm backdrop-blur">
        <div className="flex items-center justify-between text-xs">
          <span className="text-muted-foreground">
            {answered}/{all.length} itens respondidos
          </span>
        </div>
      </div>
      ...
      {complete && (
        <div className="space-y-4">
          <ClinicalReport
            scaleName="New Ballard Score"
            scaleFullName="Registro de maturidade neuromuscular e física"
            items={reportItems}
            patientAge="Recém-nascido"
          />
          <SaveToPatient
            scaleName="New Ballard Score"
