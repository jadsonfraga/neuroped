...
export default function AgendaPage() {
  ...
  const dashboard = useQuery<OperationsDashboard>({
  ...
  const patientsQuery = useQuery<{
    data: Array<{ id: string; name: string; birthDate?: string | null }>;
    total?: number;
  }>({
  ...
  useEffect(() => {
    if (!manual.serviceId && data?.services[0]?.id) {
      setManual((current) => ({ ...current, serviceId: data.services[0].id }));
    }
  }, [data?.services, manual.serviceId]);

  function beginEdit(appointment: Appointment) {
    setEditingAppointmentId(appointment.id);
    setManual({
      serviceId: appointment.serviceId,
      startsAtLocal: appointment.startsAtLocal,
  ...
  async function mutate(
    ...
    try {
      ...
      if (refreshed.isError)
        throw refreshed.error ?? new Error("Falha ao atualizar a agenda.");
      toast({ title: success });
    } catch (error) {
  ...
  const appointmentsByTime = useMemo(
    () =>
      new Map(
        dayAppointments.map(
          (item) => [item.startsAtLocal.slice(11, 16), item] as const,
        ),
      ),
    [dayAppointments],
  );
  ...
  if (dashboard.isLoading) {
    return (
      <div
        className="rounded-3xl border p-8 text-sm text-muted-foreground"
        role="status"
      >
        Carregando Agenda NeuroPed…
      </div>
    );
  }
    ...
    <div className="space-y-6 pb-16">
          ...
          <div>
            <div className="mb-3 flex flex-wrap gap-2">
              <Badge className="bg-primary/10 text-primary hover:bg-primary/10">
                NeuroPed Operacional
              </Badge>
              {data.access.delegated && (
                <Badge variant="outline">
                  Recepção vinculada · {data.access.providerName}
                </Badge>
              )}
            </div>
            ...
            <p className="mt-2 max-w-3xl text-sm leading-relaxed text-muted-foreground">
              Agenda cloud, autoagendamento, check-in, lista de espera e
              comunicação em uma única fonte. Configurações, reputação e
              financeiro permanecem restritos ao profissional responsável.
            </p>
          </div>
          <div className="flex flex-wrap gap-2">
            {canConfigure && (
      ...
      {data.access.delegated && (
        <div className="flex gap-2 rounded-2xl border border-primary/20 bg-primary/[0.06] p-3 text-xs text-muted-foreground">
          ...
          <p>
            <strong className="text-foreground">Modo recepção:</strong> você
            opera a agenda do profissional vinculado. Valores financeiros,
            configuração do serviço e moderação de avaliações não são
            disponibilizados para este perfil.
          </p>
        </div>
      )}
      ...
      <section className="grid gap-3 sm:grid-cols-2 xl:grid-cols-4">
        <Metric
          icon={CalendarClock}
          label="Hoje"
          value={String(data.metrics.today)}
        />
        <Metric
          icon={Clock3}
          label="Próximas"
          value={String(data.metrics.upcoming)}
          detail={`${data.metrics.requested} aguardando confirmação`}
        />
        {canConfigure ? (
          <Metric
            icon={CircleDollarSign}
            label="Previsto"
            value={formatMoneyBRL(data.metrics.expectedCents)}
            detail={`Recebido: ${formatMoneyBRL(data.metrics.paidCents)}`}
          />
        ) : (
          <Metric
            icon={Activity}
            label="Solicitadas"
            value={String(data.metrics.requested)}
            detail="aguardando ação da clínica"
          />
        )}
        <Metric
          icon={ListPlus}
          label="Lista de espera"
          value={String(data.metrics.waitlist)}
          detail={`${data.metrics.noShow30d} faltas em 30 dias`}
        />
      </section>
      ...
      <Tabs defaultValue="agenda" className="space-y-4">
        ...
        <TabsContent value="agenda" className="space-y-4">
            ...
            <CardHeader>
              <CardTitle className="text-base">
                {editingAppointmentId
                  ? "Editar agendamento persistente"
                  : "Novo agendamento manual"}
              </CardTitle>
              <p className="text-xs text-muted-foreground">
                A duração vem do serviço escolhido. O serviço padrão da clínica
                é de 60 minutos, em horário comercial configurado.
              </p>
            </CardHeader>
            <CardContent className="grid gap-3 md:grid-cols-2 xl:grid-cols-3">
              <Field label="Serviço">
                <select
                  ...
                  onChange={(e) =>
                    setManual((p) => ({ ...p, serviceId: e.target.value }))
                  }
                >
              ...
              <Field label="Data e horário">
                <Input
                  type="datetime-local"
                  value={manual.startsAtLocal}
                  onChange={(e) =>
                    setManual((p) => ({ ...p, startsAtLocal: e.target.value }))
                  }
                />
              </Field>
              {canConfigure && (
                <Field label="Vincular paciente ao prontuário">
                  <div className="space-y-2">
                    <Input
                      value={patientSearch}
                      onChange={(e) => setPatientSearch(e.target.value)}
                      placeholder="Buscar por nome ou identificador"
                      aria-label="Buscar paciente para vínculo clínico"
                    />
                    <select
                      className="min-h-11 w-full rounded-xl border bg-background px-3 text-sm"
                      ...
                      onChange={(e) => {
                        ...
                        setManual((p) => ({
                          ...p,
                          patientId,
                          patientName: selected?.name ?? p.patientName,
                        }));
                      }}
                    >
                      <option value="">Sem vínculo clínico</option>
                      {patientsQuery.isFetching && (
                        <option disabled>Buscando pacientes…</option>
                      )}
                    ...
                    <p className="text-[11px] text-muted-foreground">
                      {patientsQuery.data?.total ?? patientOptions.length}{" "}
              ...
              <Field label="Criança">
                <Input
                  value={manual.patientName}
                  onChange={(e) =>
                    setManual((p) => ({ ...p, patientName: e.target.value }))
                  }
                  placeholder="Nome"
                />
              </Field>
              <Field label="Responsável">
                <Input
                  ...
                  onChange={(e) =>
                    setManual((p) => ({ ...p, guardianName: e.target.value }))
                  }
                />
              </Field>
              <Field label="Telefone">
                <Input
                  value={manual.phone}
                  onChange={(e) =>
                    setManual((p) => ({ ...p, phone: e.target.value }))
                  }
                />
              </Field>
              <Field label="E-mail">
              ...
              <div className="flex flex-wrap gap-2 md:col-span-2 xl:col-span-3">
                <Button
                  disabled={busy || !manual.serviceId || !manual.startsAtLocal}
                  onClick={() =>
                    mutate(
                      editingAppointmentId
                        ? {
                            ...
                            startsAtLocal: manual.startsAtLocal,
                            patientId: canConfigure
                              ? manual.patientId || null
                              : undefined,
                            guardianName: manual.guardianName,
                            patientName: manual.patientName,
                            guardianPhone: manual.phone,
                            guardianEmail: manual.email,
                          }
                        : {
                      ...
                      editingAppointmentId
                        ? "Agendamento editado e persistido."
                        : "Consulta adicionada à agenda.",
                    )
                  }
                  className="gap-2"
                >
          ...
          <Card>
              ...
              <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
                <div>
                  ...
                  <p className="mt-1 text-xs text-muted-foreground">
                    Clique em um horário livre para preparar um novo
                    agendamento. Os registros exibidos vêm do backend
                    persistente.
                  </p>
                </div>
            ...
            <CardContent className="space-y-1">
                  ...
                  return (
                    <div
                      ...
                      onClick={() => {
                        ...
                        setManual((current) => ({
                          ...current,
                          startsAtLocal: `${agendaDate}T${time}`,
                        }));
                      }}
                      onKeyDown={(event) => {
                        if (
                      ...
                      {appointment ? (
                        <div className="flex min-w-0 flex-wrap items-center gap-2">
                          ...
                          <span className="text-xs text-muted-foreground">
                            {appointment.serviceName || "Serviço"} · até{" "}
                            {appointment.endsAtLocal.slice(11, 16)}
                          </span>
                        </div>
                      ) : (
                        <span className="text-xs text-muted-foreground">
                          Horário livre · selecionar para marcar
                        </span>
                      )}
                    </div>
                  );
              ...
              {dayAppointments.some(
                (item) =>
                  !scheduleRows.includes(item.startsAtLocal.slice(11, 16)),
              ) && (
                <p className="rounded-xl border border-amber-500/30 bg-amber-500/5 p-3 text-xs text-muted-foreground">
                  Há consultas fora da grade padrão de 07:00–20:00 ou em minutos
                  intermediários; elas continuam preservadas e aparecem na lista
                  abaixo.
                </p>
              )}
            </CardContent>
          ...
          <Card>
            <CardHeader>
              ...
              <p className="text-xs text-muted-foreground">
                {upcoming.length} consulta(s) futura(s) carregada(s).
              </p>
            </CardHeader>
            <CardContent className="space-y-2">
              {upcoming.length === 0 ? (
                <Empty text="Nenhuma consulta futura nesta agenda." />
              ) : (
                upcoming.map((apt) => (
                    ...
                    <div className="flex flex-col gap-3 lg:flex-row lg:items-center lg:justify-between">
                      <div className="min-w-0">
                        <div className="flex flex-wrap items-center gap-2">
                          <strong>
                            {apt.patientName || "Paciente não informado"}
                          </strong>
                          <Badge variant="outline">
                        ...
                        <p className="mt-1 text-sm text-muted-foreground">
                          {dateTimeLabel(apt.startsAtLocal)} · até{" "}
                          {apt.endsAtLocal.slice(11, 16)} ·{" "}
                          {apt.serviceName || "Serviço"}
                        </p>
                        <p className="mt-1 text-xs text-muted-foreground">
                          {apt.guardianName || "Responsável não informado"}
                          {apt.guardianPhone ? ` · ${apt.guardianPhone}` : ""}
                        </p>
                      </div>
                      <div className="flex flex-wrap gap-2">
                        {apt.patientId && (
                          <Button size="sm" variant="outline" asChild>
                            <Link
                              href={`/prontuario?patientId=${encodeURIComponent(apt.patientId)}`}
                            >
                        ...
                        {(nextStatuses[apt.status] ?? []).map((status) => (
                          <Button
                            key={status}
                            size="sm"
                            variant={
                            ...
                            onClick={() =>
                              (() => {
                                if (
                                  status === "cancelled" &&
                                  !window.confirm(
                                    "Cancelar este agendamento de forma permanente?",
                                  )
                                )
                                  return;
                                void mutate(
          ...
          <Card>
            ...
            <CardContent className="space-y-2">
              {data.waitlist.length === 0 ? (
                <Empty text="Ninguém na lista de espera." />
              ) : (
                data.waitlist.map((item) => (
                  ...
                  >
                    <div className="min-w-0 flex-1">
                      ...
                      <p className="text-xs text-muted-foreground">
                        {item.serviceName} · {item.guardianName}
                        ...
                        <Button
                          ...
                          onClick={() =>
                            mutate(
                              ...
                              "Horário marcado como oferecido.",
                            )
                          }
                        >
                          Oferecer
        ...
        <TabsContent value="comunicacao">
          <Card>
            <CardHeader>
              <CardTitle className="text-base">
                Caixa de saída operacional
              </CardTitle>
            </CardHeader>
            <CardContent className="space-y-3">
              <p className="rounded-xl border bg-muted/30 p-3 text-xs text-muted-foreground">
                WhatsApp, SMS e e-mail externos não são simulados. Enquanto não
                houver provedor conectado, mensagens ficam como{" "}
                <strong>pendentes</strong> e podem ser copiadas/enviadas
                manualmente.
              </p>
              {data.notifications.length === 0 ? (
                <Empty text="Nenhuma mensagem pendente." />
              ) : (
                data.notifications.map((item) => (
                  <div key={item.id} className="rounded-2xl border p-4">
                    ...
                    <p className="mt-1 text-xs text-muted-foreground">
                      Destino: {item.recipient || "não informado"}
                    </p>
                        ...
                        <Button
                          ...
                          onClick={() =>
                            mutate(
                              ...
                              "Marcada como enviada manualmente.",
                            )
                          }
                        >
                          Marcar enviada
        ...
        <TabsContent value="atividade">
          <Card>
            ...
            <CardContent className="space-y-2">
              <p className="text-xs text-muted-foreground">
                Registra quem alterou a operação sem copiar nomes de pacientes,
                telefones ou mensagens para os metadados de auditoria.
              </p>
              {data.audit.length === 0 ? (
                <Empty text="Nenhuma alteração operacional auditada ainda." />
              ) : (
                data.audit.map((entry) => (
                  <div
                    key={entry.id}
                    className="flex flex-col gap-1 rounded-xl border p-3 text-xs sm:flex-row sm:items-center sm:justify-between"
                  >
                    <div>
        ...
        {canConfigure && (
          <TabsContent value="config" className="space-y-4">
                ...
                <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-5">
                  <Field label="Nome">
                    <Input
                      ...
                      placeholder="Consulta neuropediátrica"
                    />
                  </Field>
                  <Field label="Duração (min)">
                    <Input
                      ...
                      onChange={(e) =>
                        setService((p) => ({ ...p, duration: e.target.value }))
                      }
                    />
                  </Field>
                  <Field label="Valor (R$)">
                  ...
                  <Field label="Modalidade">
                    <select
                      ...
                      onChange={(e) =>
                        setService((p) => ({ ...p, modality: e.target.value }))
                      }
                    >
                  ...
                  <div className="self-end">
                    <Button
                      className="w-full"
                      disabled={busy || !service.name.trim()}
                      onClick={() =>
                        mutate(
                          {
                    ...
                    >
                      <div className="min-w-0 flex-1">
                        ...
                        <p className="text-xs text-muted-foreground">
                          ...
                          {formatMoneyBRL(item.priceCents)} ·{" "}
                          {item.modality === "remote" ? "remota" : "presencial"}
                        </p>
                      </div>
                      <Button
                        size="sm"
                ...
                <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-5">
                  <Field label="Dia">
                    <select
                      ...
                      value={rule.weekday}
                      onChange={(e) =>
                        setRule((p) => ({ ...p, weekday: e.target.value }))
                      }
                    >
                  ...
                  <Field label="Início">
                    <Input
                      ...
                      value={rule.start}
                      onChange={(e) =>
                        setRule((p) => ({ ...p, start: e.target.value }))
                      }
                    />
                  </Field>
                  <Field label="Fim">
                    <Input
                      ...
                      value={rule.end}
                      onChange={(e) =>
                        setRule((p) => ({ ...p, end: e.target.value }))
                      }
                    />
                  </Field>
                  <Field label="Passo (min)">
                    <Input
                      type="number"
                      min="5"
                      value={rule.slot}
                      onChange={(e) =>
                        setRule((p) => ({ ...p, slot: e.target.value }))
                      }
                    />
                  </Field>
                  <div className="self-end">
                ...
                <div className="flex flex-wrap gap-2">
                  {data.rules.map((item) => (
                    ...
                    >
                      ...
                      {minutesToClock(item.startMinute)}–
                      {item.endMinute === 1440
                        ? "24:00"
                        : minutesToClock(item.endMinute)}{" "}
                      <button
                        type="button"
                        onClick={() =>
                          mutate(
                            { action: "delete_rule", id: item.id },
                            "Regra removida.",
                          )
                        }
            ...
            <Card>
              <CardHeader>
                <CardTitle className="text-base">Bloqueios e férias</CardTitle>
              </CardHeader>
              <CardContent className="space-y-4">
                <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-4">
                  <Field label="Início">
                    <Input
                      ...
                      value={block.start}
                      onChange={(e) =>
                        setBlock((p) => ({ ...p, start: e.target.value }))
                      }
                    />
                  </Field>
                  <Field label="Fim">
                    <Input
                      ...
                      value={block.end}
                      onChange={(e) =>
                        setBlock((p) => ({ ...p, end: e.target.value }))
                      }
                    />
                  </Field>
                  <Field label="Motivo">
                    <Input
                      value={block.reason}
                      onChange={(e) =>
                        setBlock((p) => ({ ...p, reason: e.target.value }))
                      }
                      placeholder="Férias, reunião…"
                    />
                  </Field>
                  <div className="self-end">
                    <Button
                      ...
                      onClick={() =>
                        mutate(
                          {
                            action: "create_block",
                            startsAtLocal: block.start,
                            endsAtLocal: block.end,
                            reason: block.reason,
                          },
                          "Bloqueio adicionado.",
                        )
                ...
                <div className="space-y-2">
                  {data.blocks.map((item) => (
                    ...
                    >
                      <span>
                        {dateTimeLabel(item.startsAtLocal)} →{" "}
                        {dateTimeLabel(item.endsAtLocal)}
                      ...
                      <Button
                        ...
                        onClick={() =>
                          mutate(
                            { action: "delete_block", id: item.id },
                            "Bloqueio removido.",
                          )
                        }
                      >
            ...
            <Card>
              <CardContent className="space-y-5 p-5 sm:p-6">
                <div className="flex items-start gap-3">
                  ...
                  <div>
                    <h2 className="font-bold">Perfil e autoagendamento</h2>
                    <p className="text-sm text-muted-foreground">
                      O booking público só abre horários quando você o ativa.
                      Contatos são cifrados antes de persistir; não há campo
                      livre para hipótese diagnóstica.
                    </p>
                  </div>
                </div>
                ...
                <div className="flex flex-wrap gap-2">
                      ...
                      mutate(
                        {
                          ...
                          ...profile,
                          bookingEnabled: data.profile.bookingEnabled,
                        },
                        "Perfil salvo.",
                      )
                    }
                  ...
                  <Button
                    variant={
                      data.profile.bookingEnabled ? "destructive" : "default"
                    }
                    disabled={busy}
                    onClick={() =>
                      mutate(
                        ...
                        data.profile.bookingEnabled
                          ? "Agendamento público pausado."
                          : "Agendamento público ativado.",
                      )
                    }
                  >
                    {data.profile.bookingEnabled
                  ...
                  <Button asChild variant="outline">
                    <Link href={publicHref}>Abrir página pública</Link>
                  </Button>
                </div>
                {!data.services.some(
                  (item) => item.active && item.publicVisible,
                ) || data.rules.length === 0 ? (
                  <p className="rounded-xl border border-amber-500/30 bg-amber-500/5 p-3 text-xs text-muted-foreground">
                    Antes de ativar, cadastre pelo menos um serviço ativo e uma
            ...
            <Card>
              ...
              <CardContent className="space-y-4">
                <p className="text-xs leading-relaxed text-muted-foreground">
                  Vincule uma conta existente cujo perfil seja{" "}
                  <code>operator</code>. O vínculo concede acesso somente à
                  agenda operacional; não concede prontuário, prescrição,
                  Clinical Core, configuração de serviços ou financeiro.
                </p>
                <div className="flex flex-col gap-2 sm:flex-row">
                  <Input
                    ...
                    placeholder="email.da.recepcao@exemplo.com"
                  />
                  <Button
                    className="gap-2"
                    disabled={busy || !staffEmail.includes("@")}
                    onClick={async () => {
                      const saved = await mutate(
                        { action: "staff_link", email: staffEmail },
                        "Recepção vinculada à agenda.",
                      );
                ...
                {data.staff.length === 0 ? (
                  <Empty text="Nenhuma conta de recepção vinculada." />
                ) : (
                        ...
                        <Button
                          ...
                          onClick={() =>
                            mutate(
                              ...
                              staff.active
                                ? "Acesso da recepção suspenso."
                                : "Acesso da recepção reativado.",
                            )
                          }
                        >
                          {staff.active ? "Suspender" : "Reativar"}
                        </Button>
                      </div>
                    </div>
                  ))
        ...
        {canConfigure && (
          <TabsContent value="reputacao">
            <Card>
              ...
              <CardContent className="space-y-3">
                <p className="text-xs text-muted-foreground">
                  Somente consultas concluídas podem gerar avaliação.
                  Comentários ficam privados até moderação.
                </p>
                {data.reviews.length === 0 ? (
                  <Empty text="Nenhuma avaliação recebida." />
                ) : (
                  data.reviews.map((item) => (
                    <div key={item.id} className="rounded-2xl border p-4">
                      <div className="flex items-center gap-2">
                        ...
                        <Badge variant="outline">
                          {item.approved ? "pública" : "aguardando moderação"}
                        </Badge>
                      </div>
                      {item.comment && (
                          ...
                          <Button
                            ...
                            onClick={() =>
                              mutate(
                                ...
                                "Avaliação retirada da página pública.",
                              )
                            }
                          >
                            Ocultar
          ...
          <TabsContent value="financeiro">
            <div className="grid gap-4 lg:grid-cols-3">
              ...
              <Metric
                icon={CheckCircle2}
                label="Recebido"
                value={formatMoneyBRL(data.metrics.paidCents)}
              />
              <Metric
                icon={MessageSquareText}
                label="Pendências de comunicação"
            ...
            <Card className="mt-4">
              ...
              <CardContent className="space-y-2">
                {data.appointments.slice(0, 100).map((apt) => (
                  <PaymentRow
                    key={apt.id}
                    appointment={apt}
                    busy={busy}
...
function PaymentRow({
  ...
  const [amount, setAmount] = useState(
    appointment.amountCents !== null
      ? String(appointment.amountCents / 100)
      : "",
  );
  return (
    <div className="grid gap-3 rounded-2xl border p-4 md:grid-cols-[minmax(0,1fr)_140px_150px_auto] md:items-end">
      <div>
        <p className="font-semibold">{appointment.patientName || "Paciente"}</p>
        <p className="text-xs text-muted-foreground">
          {dateTimeLabel(appointment.startsAtLocal)} · {appointment.serviceName}
        </p>
      </div>
      <Field label="Valor R$">
        <Input
          ...
          return mutate(
            {
              ...
              amountCents: amount
                ...
                : null,
              paymentStatus: select?.value || appointment.paymentStatus,
