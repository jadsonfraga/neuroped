...
import { patientOptions } from "@shared/escuta/patients";
...
export default function EscutaClinicaPage() {
  ...
  useEffect(()=> {
    ...
    void authFetch(`/api/live/patients?clinicId=${encodeURIComponent(activeClinicId)}`,{signal:c.signal,cache:"no-store"}).then(async r=>{const b=await r.json();if(c.signal.aborted)return;if(r.ok)setPatients(patientOptions(b.data));else setMessage(b.error || "Cadastro clínico indisponível.");}).catch(()=>{if(!c.signal.aborted)setMessage("Não foi possível carregar o cadastro clínico.");});
    return ()=>c.abort();
  },[activeClinicId]);
  async function task(work: ()=>Promise<void>) {
    ...
    try{await work();}catch(error){if(mounted.current)setMessage(error instanceof DOMException && error.name==="AbortError"?"Processamento cancelado. O áudio capturado permanece nesta aba.":error instanceof Error?error.message:"Operação indisponível.");}
  ...
  function clearAudio(){recorder.current?.destroy();transcribed.current.clear();setAudioUrl("");setState("idle");setSeconds(0);setLevel(0);}
  async function generate(text: string) {
    if(!ready || !ack)throw new Error("O processamento precisa estar habilitado e autorizado.");
    clearDraft();setProgress("Organizando a anamnese a partir da transcrição…");
  ...
  return <div className="escuta-page">
    ...
    {busy&&<p role="status" className="escuta-message">{progress||"Processando…"} {progress&&<button onClick={()=>controller.current?.abort()}>Cancelar processamento</button>}</p>}{message&&<p role="status" className="escuta-message">{message}</p>}
    ...
    <section className="escuta-card no-print"><h2>Prontuário e histórico</h2><p className="escuta-help">Acesso ao histórico não depende de gerar uma nova anamnese. Selecione cuidadosamente o paciente antes de salvar.</p>
      <div className="escuta-actions"><label>Paciente do prontuário<select disabled={busy||active} value={patientId} onChange={e=>{setPatientId(e.target.value);setHistory([]);documentId.current=null;}}><option value="">Selecione um paciente</option>{patients.map(p=><option key={p.id} value={p.id}>{p.name}</option>)}</select></label><button disabled={!patientId||busy||active||!reviewed||!note} onClick={()=>void task(async()=>{if(!note)throw new Error("Nenhum rascunho para salvar.");const saved=await post("/api/live/documents",{clinicId:activeClinicId,patientId,documentId:documentId.current,documentType:"clinical_note",origin:"clinician",status:"draft",familyVisibility:false,content:{title:"Escuta Clínica — anamnese",escutaVersion:ESCUTA_VERSION,note,transcript,reviewedAt:new Date().toISOString(),recordingAcknowledged:ack}});documentId.current=saved.id;setMessage("Rascunho salvo no prontuário clínico com a proteção já existente. Não publicado à família.");})}>Salvar rascunho revisado</button><button disabled={!patientId||busy||active} onClick={()=>void task(loadHistory)}>Consultar histórico</button></div>
      {history.map(item=><p key={item.id}>{new Date(item.updatedAt).toLocaleString("pt-BR")} <button disabled={busy||active} onClick={()=>{if(!window.confirm("Abrir esta versão e substituir o rascunho e áudio atuais?"))return;clearAudio();setTranscript(item.version.content.transcript||"");setNote(item.version.content.note||null);documentId.current=item.id;setReviewed(false);}}>Abrir versão</button></p>)}
    </section>
    <footer className="escuta-help no-print">O áudio não é salvo automaticamente. Fechar ou recarregar a aba descarta o áudio e o rascunho não salvo. Falha de processamento preserva os trechos na aba para nova tentativa. O módulo organiza o que foi dito; confira hipóteses, números e medicamentos na conversa original.</footer>
  </div>;
}
