...
const extraTools: ExtraTool[] = [
  {
    ...
    name: "Reconhecimento (2-7a)",
    description: "Cores, letras, animais e partes do corpo por faixa etária",
  ...
  },
  {
    ...
    name: "Acadêmico (5-14a)",
    description: "Leitura, escrita e aritmética por faixa etária",
  ...
  {
    id: "inventarios-auto",
    name: "Autoavaliação (8-17a)",
    description:
      "8 inventários: humor, ansiedade, atenção, sono, alimentação, social, escola, comportamento",
    route: "/inventarios-auto",
    icon: ClipboardCheck,
  ...
  },
  {
    ...
    name: "Inventários p/ Escola",
    description:
      "4 questionários para professores: comportamento, TEA, alfabetização, funções executivas",
    route: "/inventarios-escola",
    icon: School,
  ...
  {
    id: "tde2",
    name: "TDE-2 Adaptado Dr. Jadson",
    description:
      "Teste de desempenho escolar: leitura, escrita, aritmética (4-14 anos)",
    route: "/tde2",
    icon: BookOpen,
  ...
  },
  {
    ...
    description:
      "50 itens — diagnóstico diferencial superdotação vs autismo (resposta da professora)",
    route: "/ahsd-tea",
    icon: Sparkles,
  ...
  },
  {
    ...
    name: "Satisfação com Medicação",
    description:
      "Eficácia, efeitos adversos, dose, forma de administrar — resposta dos pais",
    route: "/satisfacao-medicacao",
    icon: SmilePlus,
  ...
  },
  {
    ...
    description:
      "Anamnese, marcos, medicações, terapias, exames — gera relatório completo",
    route: "/prontuario",
    icon: Stethoscope,
    gradient: "from-violet-500 to-purple-600",
    tags: [
  ...
  },
  {
    ...
    description:
      "Guia didático: o que é, quando pedir, testes, como interpretar",
  ...
  {
    id: "pac",
    name: "Processamento Auditivo",
    description: "PAC: habilidades auditivas, avaliação, diferencial com TDAH",
    route: "/pac",
    icon: Ear,
  ...
  {
    id: "avaliacao-multiprofissional",
    name: "Avaliação Multiprofissional",
    description:
      "Guia de instrumentos por faixa etária: linguagem, cognição, inteligência, TEA, aprendizagem",
    route: "/avaliacao-multiprofissional",
    icon: ClipboardCheck,
    gradient: "from-emerald-500 to-teal-600",
    tags: [
      "avaliacao",
  ...
  },
  {
    ...
    name: "Plano Terapêutico (PTI)",
    description:
      "Gerador de plano terapêutico individualizado multiprofissional",
    route: "/plano-terapeutico",
    icon: Target,
  ...
  {
    id: "plano-intervencao",
    name: "Intervenção por Habilidades",
    description:
      "Como intervir em 7 áreas: social, cognitiva, linguagem, matemática, atenção, sensorial, inclusão",
    route: "/plano-intervencao",
    icon: Lightbulb,
...
const queixas = [
  ...
  {
    id: "irritado",
    label: "Irritado / Birra",
    emoji: "😤",
    q: ["comportamento", "depressao"],
  },
...
const q2cat: Record<string, string[]> = {
  tea: ["tea"],
  tdah: ["tdah"],
  comportamento: ["tod", "comportamento"],
  ansiedade: ["ansiedade-separacao", "ansiedade-social", "tag", "ansiedade"],
  depressao: ["depressao"],
  sono: ["sono"],
  epilepsia: ["epilepsia", "crise-febril"],
  pc: ["paralisia-cerebral"],
  tiques: ["tiques"],
  enurese: ["enurese"],
  dor: ["cefaleia", "dor-neuropatica"],
  suicidio: ["depressao"],
  alimentacao: ["transtorno-alimentar"],
  cognicao: ["di"],
  atraso: ["di"],
  motor: ["movimento", "paralisia-cerebral"],
  linguagem: ["tea"],
  trauma: ["tept"],
};
...
function prioritizeGuidedScales(
  ...
  return uniqueScales(scales).sort((a, b) => {
    ...
    const aResp = respondents.some((respondent) =>
      a.respondente.includes(respondent),
    );
    const bResp = respondents.some((respondent) =>
      b.respondente.includes(respondent),
    );
...
function guidedScalePool(
  ...
  const base =
    selectedQueixas.length > 0 || ageRange !== null
  ...
  if (priorities.length > 0) {
    const byPriority = narrowed.filter((scale) =>
      priorities.includes(scale.prioridade),
    );
  ...
  return tokens.map((tk) => {
    ...
    for (const [key, vals] of Object.entries(syn)) {
      ...
      if (
        nk.includes(tk) ||
        tk.includes(nk) ||
        vals.some((v) => norm(v).includes(tk) || tk.includes(norm(v)))
      ) {
        set.add(nk);
...
function smartSearch(query: string) {
  ...
  if (!raw || raw.length < 2)
    return {
      inScales: [] as ScaleEntry[],
      inPharm: [] as { cat: PharmCategory; drug: Drug }[],
      inTools: [] as ExtraTool[],
    };
  ...
  // Escalas
  const scaleResults = allScales
    .map((s) => {
      const hay = norm(
        `${s.name} ${s.fullName} ${s.description} ${s.queixas.join(" ")}`,
      );
      let score = 0;
      for (const group of expanded) {
        if (group.some((t) => hay.includes(t))) {
          score += group.some((t) => norm(s.name).includes(t))
            ? 5
            : group.some((t) => norm(s.fullName).includes(t))
              ? 3
              : 1;
        }
      }
  ...
  for (const cat of pharmCategories) {
    for (const drug of cat.drugs) {
      const hay = norm(
        `${drug.name} ${drug.dose} ${drug.comment} ${cat.title} ${(drug.alerts || []).join(" ")}`,
      );
      let score = 0;
      for (const group of expanded) {
        if (group.some((t) => hay.includes(t))) {
...
export default function FiltroPage() {
  ...
  const results = useMemo(() => {
    ...
    if (!hasText && !hasQ && !hasAge && !hasObjective && !hasContext)
      return {
        scales: [] as ScaleEntry[],
        pharm: [] as { cat: PharmCategory; drug: Drug }[],
        tools: [] as ExtraTool[],
      };
    ...
    if (hasText) {
      ...
      if (hasQ || hasAge) {
        ...
        const boosted = resultScales.map((s) => ({
          s,
          inFilter: baseIds.has(s.id),
        }));
      ...
      if (hasQ) {
        ...
        for (const cat of pharmCategories) {
          if (
            [...catIds].some(
              (id) =>
                norm(cat.id).includes(norm(id)) ||
                norm(cat.title).includes(norm(id)),
            )
          ) {
      ...
      resultTools = extraTools.filter((tool) =>
        tool.tags.some((tag) =>
          qTexts.some(
            (qt) =>
              norm(tag).includes(norm(qt)) || norm(qt).includes(norm(tag)),
          ),
        ),
      );
    }
  ...
  const total =
    results.scales.length + results.pharm.length + results.tools.length;

  // Escala ideal (destaque dourado)
  const idealResult = useMemo(
    () => getIdealScale(techQ, ageRange),
    [techQ, ageRange],
  );
      ...
      >
        ...
        <div>
          ...
          <p className="text-xs text-muted-foreground italic">
            Fluxo guiado: idade → queixa → objetivo → contexto → instrumentos
          </p>
        </div>
      </motion.div>

      <div
        className="rounded-2xl border bg-card/80 p-3 space-y-2"
        aria-label="Etapas do filtro clínico guiado"
      >
        <div className="grid grid-cols-5 gap-1 text-center text-[10px] font-semibold">
          {[
            ...
            <div
              key={step}
              className={`rounded-lg px-1 py-1.5 ${index === 4 && total > 0 ? "bg-primary text-primary-foreground" : index === 0 && selIdade ? "bg-primary/10 text-primary" : index === 1 && selQueixas.length ? "bg-primary/10 text-primary" : index === 2 && objetivo ? "bg-primary/10 text-primary" : index === 3 && contexto ? "bg-primary/10 text-primary" : "bg-muted text-muted-foreground"}`}
            >
              {step}
            </div>
          ))}
        ...
        <p className="text-[11px] text-muted-foreground leading-relaxed">
          Não é necessário conhecer siglas: descreva a queixa ou escolha botões
          clínicos para receber opções priorizadas com idade, tempo, indicação,
          limitações e ação direta.
        </p>
      </div>
        ...
        <div className="grid grid-cols-2 gap-1.5 sm:grid-cols-4">
          {objetivos.map((o) => (
            ...
            >
              ...
              <span
                className={`block text-[10px] ${objetivo === o.id ? "text-white/80" : "text-muted-foreground"}`}
              >
                {o.hint}
              </span>
            </button>
      ...
      {!hasFilters ? (
        <div className="rounded-2xl border-2 border-dashed border-primary/15 p-6 text-center space-y-3">
          ...
          <p className="text-sm text-muted-foreground">
            Selecione <strong>idade</strong>, <strong>queixa</strong>,{" "}
            <strong>objetivo</strong> e <strong>contexto</strong> — ou busque em
            linguagem natural.
          </p>
        ...
        <div
          ...
          role="region"
          aria-live="polite"
          aria-label="Resultados do filtro inteligente"
        >
          {total > 0 && (
            <Mascote
              contexto="resultado"
          ...
          <div className="flex gap-1 bg-muted/50 rounded-xl p-1">
            {[
              ...
              {
                key: "escalas" as const,
                label: `Escalas (${results.scales.length})`,
              },
              ...(results.pharm.length > 0
                ? [
                    {
                      key: "meds" as const,
                      label: `Medicações (${results.pharm.length})`,
                    },
                  ]
                : []),
              ...(results.tools.length > 0
                ? [
                    {
                      key: "tools" as const,
                      label: `Ferramentas (${results.tools.length})`,
                    },
                  ]
                : []),
            ].map((t) => (
          ...
          {idealId &&
            tab === "tudo" &&
            (() => {
              const idealScaleEntry = results.scales.find(
                (s) => s.id === idealId,
              );
              if (!idealScaleEntry) return null;
              const mainQueixa = selQueixas[0];
              const qLabel =
                queixas.find((q) => q.id === mainQueixa)?.label || mainQueixa;
              return (
                <div className="rounded-xl bg-gradient-to-r from-amber-50 to-yellow-50 dark:from-amber-950/30 dark:to-yellow-950/20 border-2 border-amber-300/50 p-4 space-y-2">
                  ...
                  <div className="flex items-center gap-2 text-xs">
                    ...
                    <span className="px-2 py-1 rounded-lg bg-amber-200/60 dark:bg-amber-800/40 text-amber-800 dark:text-amber-200 font-bold">
                      {idealScaleEntry.name}
                    </span>
                    <ChevronRight className="w-3 h-3 text-muted-foreground" />
                    <span className="px-2 py-1 rounded-lg bg-emerald-100 dark:bg-emerald-900/40 text-emerald-700 dark:text-emerald-300 font-medium">
                      Resultado
                    </span>
                  </div>
                  {idealReason && (
          ...
          {results.scales.length > 0 && tab === "tudo" && (
            <div className="rounded-xl border border-primary/20 bg-primary/5 p-3 text-sm text-foreground">
              <p className="font-semibold">Próximo passo clínico</p>
              <p className="text-xs text-muted-foreground">
                Aplique a escala recomendada primeiro; medicações e ferramentas
                ficam como apoio, não como destino principal.
              </p>
            </div>
          )}

          {/* Escalas */}
          {(tab === "tudo" || tab === "escalas") &&
            results.scales.length > 0 && (
              <div className="space-y-2">
                ...
                <div className="space-y-1.5">
                  {results.scales
                    .slice(0, tab === "tudo" ? 10 : undefined)
                    .map((s) => (
                      <ScaleCard
                        key={s.id}
                        scale={s}
                        isIdeal={s.id === idealId}
                      />
                    ))}
          ...
          {/* Ferramentas extras — apoio secundário ao fluxo de escalas */}
          ...
          {(tab === "tudo" || tab === "meds") && results.pharm.length > 0 && (
            <div className="space-y-2">
              ...
              <div className="space-y-1.5">
                {results.pharm
                  .slice(0, tab === "tudo" ? 8 : undefined)
                  .map((r, i) => (
                    <MedCard
                      key={`${r.cat.id}-${r.drug.name}-${i}`}
                      cat={r.cat}
                      drug={r.drug}
                    />
                  ))}
    ...
    <Link href={tool.route}>
      <Card className="border-primary/20 bg-gradient-to-r from-primary/5 to-transparent cursor-pointer hover:shadow-md group transition-all card-hover">
        <CardContent className="p-3 flex items-center gap-3">
          <div
            className={`w-10 h-10 rounded-xl bg-gradient-to-br ${tool.gradient} flex items-center justify-center flex-shrink-0 shadow-sm`}
          >
...
const goldRules: GoldRule[] = [
  // ── TEA por faixa etária (altamente específicas) ──
  {
    ...
    scaleId: "mchat",
    score: 100,
    reason: "M-CHAT-R/F é padrão-ouro de triagem de TEA entre 16-30 meses",
  },
  {
    queixas: ["tea"],
    ageMin: 0,
    ...
    score: 95,
    reason:
      "Antes de 16 meses: triagem de regulação sensorial e marcos precoces",
  },
  {
    queixas: ["tea"],
    ageMin: 31,
    ...
    score: 98,
    reason:
      "CARS-2 é referência para classificar gravidade de TEA em pré-escolares",
  },
  {
    queixas: ["tea"],
    ageMin: 73,
    ...
    scaleId: "srs2",
    score: 95,
    reason: "SRS-2 quantifica responsividade social — ideal em escolares",
  },
  {
    queixas: ["tea"],
    ageMin: 145,
    ...
    scaleId: "assq",
    score: 90,
    reason: "ASSQ detecta TEA de alto funcionamento em adolescentes",
  },
  ...
  // ── TDAH por faixa etária ──
  {
    ...
    score: 100,
    reason:
      "SNAP-IV é o instrumento de triagem mais validado para TDAH escolar",
  },
  {
    queixas: ["tdah"],
    ageMin: 72,
    ...
    score: 90,
    reason:
      "Vanderbilt NICHQ — triagem com domínios de desatenção e hiperatividade",
  },
  {
    queixas: ["tdah"],
    ageMin: 36,
    ...
    score: 85,
    reason:
      "SDQ cobre hiperatividade em pré-escolares quando SNAP-IV não se aplica",
  },
  {
    queixas: ["tdah"],
    ageMin: 145,
    ...
    scaleId: "conners",
    score: 95,
    reason: "Conners-3 é mais completo para adolescentes com suspeita de TDAH",
  },
  ...
  // ── Ansiedade por faixa etária ──
  {
    ...
    scaleId: "scared",
    score: 100,
    reason: "SCARED é padrão-ouro para triagem de ansiedade a partir de 8 anos",
  },
  {
    queixas: ["ansiedade"],
    ageMin: 48,
    ...
    scaleId: "scas",
    score: 90,
    reason: "SCAS cobre ansiedade em crianças mais novas (4-7 anos)",
  },
  {
    queixas: ["ansiedade"],
    ageMin: 36,
    ...
    scaleId: "eai",
    score: 85,
    reason: "EAI-J26 para triagem de ansiedade em crianças pequenas",
  },
  ...
  // ── Depressão por faixa etária ──
  {
    ...
    score: 100,
    reason:
      "CDI-2 é o inventário padrão de depressão para crianças de 7-12 anos",
  },
  {
    queixas: ["depressao"],
    ageMin: 132,
    ...
    score: 98,
    reason:
      "PHQ-A é validado para depressão em adolescentes a partir de 11 anos",
  },
  {
    queixas: ["depressao"],
    ageMin: 48,
    ...
    score: 90,
    reason:
      "EDI-J26 adaptado para triagem de depressão em pré-escolares e escolares iniciais",
  },
  ...
  // ── Atraso do desenvolvimento ──
  {
    ...
    score: 100,
    reason:
      "Denver II é o triagem clássico de marcos do desenvolvimento até 6 anos",
  },
  {
    queixas: ["atraso"],
    ageMin: 1,
    ...
    scaleId: "asq3",
    score: 95,
    reason: "ASQ-3 triagem com questionário para pais — 5 domínios",
  },
  {
    queixas: ["atraso"],
    ageMin: 0,
    ...
    scaleId: "j26-001",
    score: 92,
    reason: "Protocolo de triagem motora precoce para o primeiro ano",
  },
  ...
  // ── Comportamento ──
  {
    ...
    scaleId: "sdq",
    score: 100,
    reason: "SDQ é a triagem comportamental mais rápida e validada",
  },
  {
    queixas: ["comportamento"],
    ageMin: 48,
    ...
    score: 90,
    reason:
      "CBCL é o padrão-ouro para avaliação abrangente de problemas comportamentais",
  },

  // ── Epilepsia ──
  {
    queixas: ["epilepsia"],
    scaleId: "epilepsia-diario",
    score: 100,
    reason:
      "Diário de crises é essencial para quantificar e caracterizar episódios",
  },
  {
    queixas: ["epilepsia"],
    scaleId: "chalfont",
    score: 85,
    reason: "Escala de gravidade de crises de Chalfont para monitorização",
  },
  ...
  // ── Sono ──
  {
    ...
    score: 100,
    reason:
      "CSHQ é o questionário mais completo para hábitos de sono em crianças",
  },
  {
    queixas: ["sono"],
    ageMin: 0,
    ...
    scaleId: "bisq",
    score: 90,
    reason: "BISQ para triagem rápida de sono em lactentes",
  },
  {
    queixas: ["sono"],
    ageMin: 121,
    ...
    scaleId: "j26-130",
    score: 88,
    reason: "Protocolo de higiene do sono para adolescentes",
  },
  ...
  // ── Linguagem ──
  {
    ...
    ageMin: 0,
    ageMax: 36,
    scaleId: "cdi-macarthur",
    score: 100,
    reason: "CDI MacArthur é padrão para vocabulário e gestos em lactentes",
  },
  {
    queixas: ["linguagem"],
    ageMin: 37,
    ...
    scaleId: "pls5",
    score: 95,
    reason: "PLS-5 avalia linguagem receptiva e expressiva em pré-escolares",
  },
  {
    queixas: ["linguagem"],
    ageMin: 73,
    ...
    scaleId: "celf5",
    score: 90,
    reason: "CELF-5 para avaliação abrangente de linguagem em escolares",
  },
  ...
  // ── Motor ──
  {
    ...
    scaleId: "gmfcs",
    score: 100,
    reason: "GMFCS classifica função motora grossa na paralisia cerebral",
  },
  {
    queixas: ["motor"],
    ageMin: 36,
    ...
    scaleId: "mabc2",
    score: 88,
    reason: "MABC-2 avalia coordenação motora em crianças escolares",
  },
  {
    queixas: ["motor"],
    ageMin: 0,
    ...
    scaleId: "aims",
    score: 92,
    reason: "AIMS avalia motricidade em lactentes de risco",
  },
  ...
  // ── Aprendizagem ──
  {
    ...
    scaleId: "tde",
    score: 100,
    reason: "TDE-II é o teste padronizado brasileiro de desempenho escolar",
  },
  {
    queixas: ["aprendizagem"],
    ageMin: 72,
    ...
    scaleId: "prolec",
    score: 88,
    reason: "PROLEC avalia processos de leitura — triagem de dislexia",
  },
  ...
  // ── Cognição / QI ──
  {
    ...
    scaleId: "wisc5",
    score: 100,
    reason: "WISC-V é padrão-ouro para avaliação de inteligência em escolares",
  },
  {
    queixas: ["cognicao"],
    ageMin: 30,
    ...
    scaleId: "wppsi",
    score: 95,
    reason: "WPPSI-IV para avaliação de inteligência em pré-escolares",
  },
  {
    queixas: ["cognicao"],
    ageMin: 48,
    ...
    scaleId: "raven",
    score: 85,
    reason: "Raven Colorido como triagem rápida de raciocínio não-verbal",
  },
  ...
  // ── Tiques ──
  {
    ...
    scaleId: "ygtss",
    score: 100,
    reason: "YGTSS é o padrão-ouro para gravidade de tiques motores e vocais",
  },
  ...
  // ── Suicídio / Autolesão ──
  {
    ...
    scaleId: "cssrs",
    score: 100,
    reason: "C-SSRS é o padrão mundial para avaliação de risco suicida",
  },
  {
    queixas: ["suicidio"],
    scaleId: "ecar-si",
    score: 90,
    reason: "ECAR-SI J26 para avaliação expandida de autoagressão em crianças",
  },
  ...
  // ── TOC ──
  {
    ...
    scaleId: "cybocs",
    score: 100,
    reason: "CY-BOCS é padrão-ouro para gravidade de TOC em crianças",
  },
  {
    queixas: ["toc"],
    scaleId: "oci-cv",
    score: 85,
    reason: "OCI-CV triagem rápida de sintomas obsessivo-compulsivos",
  },
  ...
  // ── Trauma ──
  {
    ...
    scaleId: "ucla-ptsd",
    score: 100,
    reason: "UCLA PTSD-RI é referência para TEPT em crianças e adolescentes",
  },
  {
    queixas: ["trauma"],
    scaleId: "cats",
    score: 85,
    reason: "CATS triagem de exposição e resposta traumática",
  },
  ...
  // ── Alimentação ──
  {
    ...
    scaleId: "soma",
    score: 100,
    reason: "SOMA avalia função motora oral durante alimentação",
  },
  {
    queixas: ["alimentacao"],
    ageMin: 24,
    ...
    score: 90,
    reason:
      "ETARE-J26 para triagem de transtorno alimentar restritivo evitativo",
  },
  ...
  // ── Funcionalidade ──
  {
    ...
    scaleId: "vineland",
    score: 100,
    reason: "Vineland-3 é referência para comportamento adaptativo",
  },
  {
    queixas: ["funcionalidade"],
    scaleId: "eaf",
    score: 85,
    reason: "EAF-J26 para adaptação funcional em contextos diários",
  },

  // ── Dor ──
  {
    queixas: ["dor"],
    scaleId: "cefaleia-calendario",
    score: 100,
    reason: "Calendário de cefaleia para registro e acompanhamento",
  },
  {
    queixas: ["dor"],
    ageMin: 0,
    ...
    scaleId: "flacc",
    score: 95,
    reason: "FLACC para avaliação de dor em crianças pré-verbais",
  },
  {
    queixas: ["dor"],
    ageMin: 84,
    ...
    scaleId: "eva-ped",
    score: 88,
    reason: "EVA pediátrica para autoavaliação de dor",
  },
  ...
  // ── Substâncias ──
  {
    ...
    scaleId: "crafft",
    score: 100,
    reason: "CRAFFT é o triagem padrão de uso de substâncias em adolescentes",
  },
  ...
  // ── Neonatal ──
  {
    ...
    scaleId: "apgar",
    score: 100,
    reason: "APGAR Score para avaliação imediata do recém-nascido",
  },
  {
    queixas: ["neonatal"],
    scaleId: "gma",
    score: 92,
    reason:
      "GMA para avaliação de movimentos generalizados — preditor precoce de PC",
  },

  // ── Psicose ──
  {
    queixas: ["psicose"],
    scaleId: "prime-screen",
    score: 100,
    reason: "PRIME Screen para triagem de risco ultra-alto de psicose",
  },
  ...
  // ── Enurese ──
  {
    ...
    scaleId: "dvss",
    score: 100,
    reason: "DVSS para avaliação de disfunção miccional",
  },
  {
    queixas: ["enurese"],
    scaleId: "bladder-diary",
    score: 90,
    reason: "Diário vesical pediátrico para registro de padrão urinário",
  },
  ...
  // ── Efeitos colaterais ──
  {
    ...
    scaleId: "uku",
    score: 100,
    reason: "UKU para monitorização abrangente de efeitos colaterais",
  },
  ...
  // ── Sensorial ──
  {
    ...
    scaleId: "ips",
    score: 100,
    reason: "IPS-J26 para perfil de processamento sensorial",
  },
  {
    queixas: ["sensorial"],
    scaleId: "j26-118",
    score: 85,
    reason: "Protocolo de processamento tátil Dr. Jadson",
  },
  ...
  // ── Social ──
  {
    ...
    scaleId: "ecsm",
    score: 100,
    reason: "ECSM-J26 para cognição social e mentalização",
  },
  {
    queixas: ["social"],
    scaleId: "easi",
    score: 88,
    reason: "EASI-J26 para ansiedade social infantil",
  },
  ...
  // ── Autonomia ──
  {
    ...
    scaleId: "vineland",
    score: 100,
    reason: "Vineland-3 para habilidades adaptativas e autonomia",
  },
  {
    queixas: ["autonomia"],
    scaleId: "eaf",
    score: 90,
    reason: "EAF-J26 para adaptação funcional em AVDs",
  },
  ...
  // ── Evolução / Reavaliação ──
  {
    ...
    score: 100,
    reason:
      "Registro evolutivo de comunicação social em TEA — compara com consulta anterior",
  },
  {
    queixas: ["evolucao", "tdah"],
    scaleId: "j26-219",
    score: 100,
    reason: "SNAP-IV de monitorização — compara pré e pós-medicação",
  },
  {
    queixas: ["evolucao", "epilepsia"],
    scaleId: "j26-227",
    score: 100,
    reason: "Diário de frequência de crises — acompanhamento mensal",
  },
  {
    queixas: ["evolucao", "linguagem"],
    scaleId: "j26-233",
    score: 100,
    reason: "Evolução de vocabulário expressivo — acompanhamento trimestral",
  },
  {
    queixas: ["evolucao", "motor"],
    scaleId: "j26-238",
    score: 100,
    reason: "Evolução de classificação motora GMFCS",
  },
  {
    queixas: ["evolucao", "pc"],
    scaleId: "j26-238",
    score: 100,
    reason: "Evolução motora em paralisia cerebral",
  },
  {
    queixas: ["evolucao", "aprendizagem"],
    scaleId: "j26-244",
    score: 100,
    reason: "Evolução de leitura e fluência — acompanhamento escolar",
  },
  {
    queixas: ["evolucao", "ansiedade"],
    scaleId: "j26-249",
    score: 100,
    reason: "Monitorização de ansiedade pós-tratamento",
  },
  {
    queixas: ["evolucao", "depressao"],
    scaleId: "j26-250",
    score: 100,
    reason: "Evolução de sintomas depressivos — comparação sequencial",
  },
  {
    queixas: ["evolucao", "efeitos"],
    scaleId: "j26-255",
    score: 100,
    reason: "Monitorização metabólica de antipsicóticos",
  },
  {
    queixas: ["evolucao"],
    scaleId: "j26-259",
    score: 80,
    reason: "CGI — Impressão Clínica Global de gravidade e melhora",
  },
];
...
function ScaleCard({
  ...
  const favButton = (
    <button
      ...
      aria-label={
        fav
          ? `Remover ${scale.name} dos favoritos`
          : `Adicionar ${scale.name} aos favoritos`
      }
      title={fav ? "Remover dos favoritos" : "Adicionar aos favoritos"}
      className="flex-shrink-0 p-1 rounded-md hover:bg-muted transition-colors"
    >
      <Star
        className={`w-3.5 h-3.5 transition-colors ${fav ? "fill-amber-400 text-amber-400" : "text-muted-foreground"}`}
      />
    </button>
  );
  ...
  const inner = (
    ...
    >
      <CardContent className="p-3 flex items-start gap-2.5">
        ...
        <div
          className={`w-1.5 rounded-full self-stretch flex-shrink-0 ${isIdeal ? "bg-amber-400" : scale.prioridade === "triagem" ? "bg-blue-400" : scale.prioridade === "diagnostica" ? "bg-purple-400" : "bg-green-400"}`}
        />
        <div className="flex-1 min-w-0 space-y-0.5">
          <div className="flex items-center gap-1.5 flex-wrap">
            <h3 className="text-sm font-bold text-foreground">{scale.name}</h3>
            <Badge className={`text-[10px] h-4 px-1.5 ${prio.color}`}>
              {prio.label}
            </Badge>
            {hasRoute && (
          ...
          <div className="mt-2 grid gap-1 text-[10px] text-muted-foreground sm:grid-cols-2">
            <span>
              ...
              {scale.prioridade === "triagem"
                ? "primeira triagem ou rastreio"
                : scale.prioridade === "diagnostica"
                  ? "avaliação clínica estruturada"
                  : "monitorar resposta/evolução"}
              .
            </span>
            <span>
              <strong>Evitar:</strong> uso isolado como diagnóstico definitivo.
            </span>
          </div>
          {hasRoute && (
            <div className="mt-2 flex flex-wrap gap-1.5">
              ...
              <span className="rounded-lg border px-2 py-1 text-[10px] font-medium text-muted-foreground">
                Ver detalhes
              </span>
              <span className="rounded-lg border px-2 py-1 text-[10px] font-medium text-muted-foreground">
                Salvar favorito
...
function MedCard({ cat, drug }: { cat: PharmCategory; drug: Drug }) {
  ...
  return (
    <Card className="border-card-border">
      <CardContent className="p-3 flex items-start gap-2.5">
        <div
          className={`w-7 h-7 rounded-lg bg-gradient-to-br ${cat.gradient} flex items-center justify-center flex-shrink-0`}
        >
