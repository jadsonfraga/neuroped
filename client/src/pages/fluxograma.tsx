...
function PickChip({
  ...
  const inner = (
    <div
      className={`flex items-center justify-between gap-2 rounded-xl border px-2.5 py-1.5 transition ${base} ${route ? "cursor-pointer hover:shadow-sm hover:brightness-[1.02]" : "opacity-90"}`}
    >
      <div className="min-w-0">
        ...
        {pick.sub && (
          <p className="text-[10px] font-semibold leading-tight">{pick.sub}</p>
        )}
      </div>
      {route ? (
        <ArrowRight className="h-3.5 w-3.5 shrink-0 opacity-70" />
      ) : (
        <span className="shrink-0 rounded-full border border-current/40 px-1.5 py-0.5 text-[9px] font-bold uppercase tracking-wide">
          ref
        </span>
      )}
    </div>
      ...
      <div className="flex items-start gap-2.5">
        ...
        <div className="min-w-0 flex-1">
          ...
          <div className="mt-3 grid gap-1.5 sm:grid-cols-3">
            {node.picks.map((p) => (
              <PickChip key={p.label} pick={p} variant="ouro" />
            ))}
          </div>
        </div>
      </div>
    ...
    <div className="page-enter pb-8 space-y-5">
      ...
      <header className="relative overflow-hidden rounded-3xl border border-border/60 bg-gradient-to-b from-primary/[0.06] via-card/40 to-card/20 p-5 sm:p-7 shadow-sm backdrop-blur">
        ...
        <div className="relative flex items-center gap-3.5">
          ...
          <div className="min-w-0 flex-1">
            ...
            <h1 className="mt-0.5 text-xl sm:text-[28px] font-semibold leading-tight tracking-[-0.01em] text-foreground">
              🧠 Fluxograma Clínico Inteligente
            </h1>
            <p className="mt-1 text-[13px] sm:text-sm leading-relaxed text-muted-foreground">
              Cruzamento idade × queixa × respondente × finalidade. Toque na{" "}
              <strong className="text-foreground">queixa</strong> para abrir o
              Filtro já preenchido; toque num{" "}
              <strong className="text-foreground">chip</strong> para abrir a
              escala.
            </p>
          </div>
        </div>
      ...
      <section className="rounded-2xl border border-primary/20 bg-gradient-to-br from-primary/5 via-transparent to-chart-2/5 p-4 sm:p-5">
        <p className="flex items-center gap-1.5 text-[11px] font-semibold uppercase tracking-[0.14em] text-primary">
          <Sparkles className="h-3.5 w-3.5" /> Regra-mestre
        </p>
                  ...
                  <div key={cell.queixa} className="space-y-1.5">
                    {cell.queixaId ? (
                      <button
                        type="button"
                        onClick={() =>
                          openInFilter(band.ageBandId, cell.queixaId)
                        }
      ...
      <Card className="border-amber-200/70 bg-amber-50/70 dark:border-amber-900/40 dark:bg-amber-950/20">
        <CardContent className="p-4 text-xs leading-relaxed text-amber-900 dark:text-amber-100">
          <strong>Leitura prudente:</strong> o fluxograma organiza e prioriza
          instrumentos por perfil — é apoio à decisão, não substitui o
          julgamento clínico. Chips marcados <strong>ref</strong> indicam escala
          de referência ainda não preenchível no app; os demais abrem a
          aplicação. O nó de segurança tem precedência sobre qualquer queixa.
        </CardContent>
      </Card>
    </div>
  );
