    ...
    <div className="mx-auto max-w-7xl space-y-6 px-4 py-8">
      ...
      <Tabs defaultValue="onboarding" className="space-y-4">
        ...
        <TabsContent value="partners" className="space-y-4">
          <Card><CardHeader><CardTitle>Novo parceiro de cuidado</CardTitle></CardHeader><CardContent><div className="grid gap-4 md:grid-cols-4"><Field label="Nome"><Input value={partnerName} onChange={(event) => setPartnerName(event.target.value)} placeholder="Clínica, escola ou profissional" /></Field><Field label="Tipo"><select className="h-10 rounded-md border bg-background px-3 text-sm" value={partnerType} onChange={(event) => setPartnerType(event.target.value)}>{Object.entries(typeLabels).map(([value, label]) => <option key={value} value={value}>{label}</option>)}</select></Field><Field label="Contato"><Input value={partnerContact} onChange={(event) => setPartnerContact(event.target.value)} placeholder="Nome do contato" /></Field><Field label="E-mail"><Input type="email" value={partnerEmail} onChange={(event) => setPartnerEmail(event.target.value)} placeholder="contato@exemplo.com" /></Field></div><Button className="mt-4" disabled={!partnerName.trim() || mutation.isPending} onClick={() => mutation.mutate({ path: "/api/partners", payload: { action: "create_partner", name: partnerName, type: partnerType, contactName: partnerContact, email: partnerEmail } })}>Adicionar parceiro</Button></CardContent></Card>
        ...
        <TabsContent value="communication" className="space-y-4">
          <Card><CardHeader><CardTitle>Criar template reutilizável</CardTitle></CardHeader><CardContent><div className="grid gap-4 md:grid-cols-3"><Field label="Nome"><Input value={templateName} onChange={(event) => setTemplateName(event.target.value)} placeholder="Lembrete de retorno" /></Field><Field label="Chave"><Input value={templateKey} onChange={(event) => setTemplateKey(event.target.value.toLowerCase().replace(/[^a-z0-9_-]/g, ""))} placeholder="retorno_lembrete" /></Field><Field label="Corpo"><Input value={templateBody} onChange={(event) => setTemplateBody(event.target.value)} placeholder="Olá, {{recipient_name}}…" /></Field></div><Button className="mt-4" disabled={!templateName || !templateKey || !templateBody || mutation.isPending} onClick={() => mutation.mutate({ path: "/api/communication", payload: { action: "create_template", name: templateName, templateKey, body: templateBody, channels: ["email", "whatsapp"] } })}>Salvar template</Button></CardContent></Card>
          <Card><CardHeader><CardTitle>Enfileirar comunicação</CardTitle></CardHeader><CardContent><div className="grid gap-4 md:grid-cols-3"><Field label="Template"><select className="h-10 rounded-md border bg-background px-3 text-sm" value={deliveryTemplateId} onChange={(event) => setDeliveryTemplateId(event.target.value)}><option value="">Selecione</option>{(communication.data?.templates ?? []).map((template) => <option key={template.id} value={template.id}>{template.name}</option>)}</select></Field><Field label="Canal"><select className="h-10 rounded-md border bg-background px-3 text-sm" value={deliveryChannel} onChange={(event) => setDeliveryChannel(event.target.value)}><option value="email">E-mail</option><option value="whatsapp">WhatsApp</option><option value="sms">SMS</option></select></Field><Field label="Destinatário"><Input value={deliveryRecipient} onChange={(event) => setDeliveryRecipient(event.target.value)} placeholder="E-mail ou telefone" /></Field></div><Button className="mt-4" disabled={!deliveryTemplateId || !deliveryRecipient || mutation.isPending} onClick={() => mutation.mutate({ path: "/api/communication", payload: { action: "queue_delivery", templateId: deliveryTemplateId, channel: deliveryChannel, recipient: deliveryRecipient } })}><Send className="mr-2 h-4 w-4" /> Enfileirar</Button></CardContent></Card>
