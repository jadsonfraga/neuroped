...
export default function EaahPage() {
  ...
  if (showResult) {
    const labelFor = (item: { key: string; domainIdx: number }) => {
      ...
      const labels =
        item.domainIdx === PROTECTIVE_DOMAIN_IDX
          ? eaahProtectiveLabels
          : eaahLabels;
        ...
        <div className="rounded-xl bg-red-50 dark:bg-red-950/20 border border-red-200 dark:border-red-800/40 p-4">
          <div className="flex items-start gap-2">
            ...
            <p className="text-xs text-red-800 dark:text-red-300 leading-relaxed">
              Instrumento de triagem. Em caso de risco de autoagressão ou
              heteroagressão, aciones avaliação de segurança imediata (SAMU 192
              / CVV 188).
            </p>
          </div>
        </div>
    ...
    <div className="space-y-6">
      <div className="flex items-center gap-3">
        ...
        <div>
          ...
          <p className="text-xs text-muted-foreground">
            Escala de Autoagressividade e Heteroagressividade
          </p>
        </div>
      </div>
      ...
      <div className="rounded-xl bg-blue-50 dark:bg-blue-950/20 border border-blue-200 dark:border-blue-800/40 p-4">
        <p className="text-xs text-blue-800 dark:text-blue-300 leading-relaxed">
          <strong>Instruções:</strong> Para cada item, indique a frequência. O
          Domínio C (Fatores Protetores) usa escala invertida — pontuações mais
          altas indicam melhor proteção.
        </p>
      </div>
        ...
        return (
          <div key={di} className="space-y-3">
            <div className="flex items-center gap-2 py-2">
              <div
                className={`w-3 h-3 rounded-full ${isProtective ? "bg-emerald-500" : domain.color.includes("red") ? "bg-red-500" : "bg-orange-500"}`}
              />
