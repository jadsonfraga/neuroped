    ...
    return (
      <div className="space-y-6">
            ...
            <div className="space-y-3">
              {cdi2Questions.map((q, i) => (
                <div key={i} className="rounded-lg bg-muted/30 p-3 space-y-1">
                  ...
                  <p className="text-sm font-medium text-primary pl-8">→ {answers[i] !== undefined ? cdi2Labels[answers[i]] : "—"}</p>
                </div>
              ))}
            </div>
          </CardContent>
        ...
        <ClinicalReport
          ...
          items={cdi2Questions.map((q, i) => ({ question: q, answer: answers[i] !== undefined ? cdi2Labels[answers[i]] : "—", value: answers[i] ?? 0 }))}
