...
function makeDraftKey(title: string) {
  return `neuroped:scale-draft:${title.replace(/\s+/g, "-").toLowerCase()}`;
}

export function GenericScale({ config }: { config: ScaleConfig }) {
  const draftKey = useMemo(() => makeDraftKey(config.title), [config.title]);
