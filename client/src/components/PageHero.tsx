...
interface PageHeroProps {
  ...
  /** Figurinha kawaii no canto do hero (cameo afetivo, decorativo). */
  sticker?: KawaiiStickerName;
  children?: ReactNode;
}

/** Figurinha determinística pelo título, para todo hero ter um cameo kawaii. */
function stickerFromTitle(title: string): KawaiiStickerName {
  const pool: KawaiiStickerName[] = [
    "estrela",
    "nuvem",
    "coracao",
    "cerebro",
 ...
 * Um cameo kawaii discreto no canto dá o toque afetivo da marca em cada módulo.
 */
export function PageHero({
  ...
  const heroSticker = sticker ?? stickerFromTitle(title);
  return (
    <header className="np-page-hero relative overflow-hidden rounded-[1.4rem] border border-border/80 bg-card/92 p-5 shadow-[0_1px_1px_hsl(var(--foreground)/0.025),0_16px_42px_-34px_hsl(var(--foreground)/0.38)] sm:p-6">
      ...
      {!mascotContext && (
        <div
          className="pointer-events-none absolute -right-1.5 -bottom-2 hidden opacity-70 sm:block"
          aria-hidden="true"
        >
          <KawaiiSticker name={heroSticker} size={46} float />
        </div>
      )}
