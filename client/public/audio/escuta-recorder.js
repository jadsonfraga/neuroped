...
class EscutaRecorder extends AudioWorkletProcessor {
  constructor() {
    ...
    this.samples = new Int16Array(16000); this.offset = 0; this.total = 0; this.peak = 0; this.energy = 0;
  ...
  flush() {
    ...
    const pcm = this.samples.slice(0, this.offset); const count = this.offset; this.offset = 0;
    const peak = this.peak; const rms = Math.sqrt(this.energy / Math.max(1, count)); this.peak = 0; this.energy = 0;
    this.port.postMessage({ pcm, total: this.total, peak, rms }, [pcm.buffer]);
  }
  process(inputs) {
    ...
    for (const value of input) {
      ...
      if (this.phase >= sampleRate) {
        ...
        this.samples[this.offset++] = Math.round(normalized * (normalized < 0 ? 32768 : 32767)); this.peak = Math.max(this.peak, Math.abs(normalized)); this.energy += normalized * normalized;
