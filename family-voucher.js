(function(){
  ...
  function saveList(x){try{localStorage.setItem(LIST_KEY,JSON.stringify(x));return true}catch(e){return false}}
  ...
  async function validateVoucher(code){const hash=await sha(norm(code));const list=loadList();const found=list.find(x=>x.hash===hash);if(!found)return {ok:false,reason:'Voucher não encontrado neste aparelho.'};if(now()>found.expiresAt)return {ok:false,reason:'Voucher expirado em '+dateBR(found.expiresAt)+'.'};const s={ok:true,type:'family-voucher',hash,startedAt:now(),expiresAt:found.expiresAt,child:found.child||'',family:found.family||''};try{localStorage.setItem(KEY,JSON.stringify(s))}catch(e){return {ok:false,reason:'Não foi possível salvar a sessão (armazenamento cheio ou bloqueado).'}}return {ok:true,session:s,expiresText:dateBR(found.expiresAt)}}
