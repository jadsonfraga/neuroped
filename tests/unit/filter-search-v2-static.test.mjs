...
for (const file of ["client/src/lib/scaleSearch.ts", "client/src/lib/filterDiagnostics.ts", "client/src/lib/filterUrlState.ts", "client/src/data/scaleSearchAliases.ts"]) {
  const src = readFileSync(file, "utf8");
  assert.ok(!/from "react"/.test(src), `${file} é puro (sem React)`);
}
// A suíte de filtro executa os testes novos.
const pkg = JSON.parse(readFileSync("package.json", "utf8"));
for (const needle of ["tests/clinical/test-filter-search-quality.mjs", "tests/unit/filter-query-intent.test.ts", "tests/unit/filter-diagnostics.test.ts", "tests/unit/filter-url-state.test.ts", "tests/unit/filter-search-v2-static.test.mjs"]) {
