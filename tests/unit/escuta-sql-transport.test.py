"""Verify transport against the complete real migration corpus, not a fake schema."""
...
for path in files:
    ...
    for statement in split_sql(source):
        b.execute(statement)
        count += 1
names = lambda db: db.execute('SELECT type,name,tbl_name FROM sqlite_master ORDER BY type,name').fetchall()
...
print(f'PASS canonical SQL transport: {len(files)} files, {count} statements, {len(names(a))} schema objects; comments/literals/triggers preserved.')
