...
import { onRequestGet as profileGet, onRequestPut as profilePut } from "../../functions/api/me/profile";
import { onRequestGet as tenantGet, onRequestPatch as tenantPatch } from "../../functions/api/tenants/[id]/index";
...
interface FakeContextInput {
  db: D1Database;
  request: Request;
  user?: { id: string; email: string; name: string; role: string; mustChangePassword: boolean } | null;
...
// ── 1. Signup ───────────────────────────────────────────────────────────────
{
  ...
  assert.equal(closed.status, 503, "signup deve nascer fechado (decisão de negócio do operador)");
  assert.equal((await closed.json() as { code: string }).code, "SIGNUP_DISABLED");

  const enabledEnv = { SAAS_SIGNUP_ENABLED: "true", ADMIN_EMAIL: "dono@plataforma.dev" };
  ...
  const created = await ok.json() as { accessToken: string; refreshToken: string; user: { role: string; email: string } };
  assert.ok(created.accessToken && created.refreshToken, "signup emite sessão revogável");
  assert.equal(created.user.role, "professional");
  assert.equal(created.user.email, "nova@example.com", "e-mail normalizado");
  const row = raw.prepare("SELECT role, is_active, password_hash FROM users WHERE email='nova@example.com'").get() as {
    role: string; is_active: number; password_hash: string;
  };
  assert.equal(row.role, "professional");
  assert.equal(row.is_active, 1);
  assert.ok(row.password_hash && !row.password_hash.includes(STRONG_PASSWORD), "senha nunca em texto claro");
  ...
  for (const email of ["nova@example.com", "dono@plataforma.dev"]) {
    ...
    assert.equal(dup.status, 409, `cadastro repetido/reservado (${email}) responde 409`);
...
// ── 2. Tenant settings: gestão, RBAC e anti-IDOR ────────────────────────────
{
  ...
  `);
  const owner = { id: "u-owner", email: "a@x.dev", name: "Dona A", role: "professional", mustChangePassword: false };
  const prof = { id: "u-prof", email: "p@x.dev", name: "Prof A", role: "professional", mustChangePassword: false };
  const other = { id: "u-other", email: "b@x.dev", name: "Dona B", role: "professional", mustChangePassword: false };

  const patchBody = {
    name: "Clínica A Renomeada",
    timezone: "America/Sao_Paulo",
    settings: { displayName: "Clínica A", addressLine1: "Rua Um, 10", phone: "(11) 90000-0000" },
  };
  ...
  const cross = await tenantPatch(
    makeContext({
      db,
      request: jsonRequest("https://x.test/api/tenants/clin-a", "PATCH", patchBody),
      user: other,
      params: { id: "clin-a" },
    }) as never,
  );
  assert.equal(cross.status, 403, "membro de outra clínica não pode alterar o tenant");
  ...
  assert.equal(nonManager.status, 403, "professional sem gestão não altera settings");
  ...
  const patched = await okPatch.json() as { name: string; timezone: string; settings: { displayName: string } };
  ...
  assert.ok(
    raw.prepare("SELECT 1 FROM saas_audit_log WHERE clinic_id='clin-a' AND action='clinic_update'").get(),
  ...
  assert.equal(crossGet.status, 403, "não-membro não lê nem os settings do tenant");
}
...
// ── 3. Perfil profissional próprio ──────────────────────────────────────────
{
  const raw = freshDb();
  const db = makeDb(raw);
  raw.exec(`INSERT INTO users(id, name, email, role) VALUES ('u-1', 'Ana', 'ana@x.dev', 'professional');`);
  const ana = { id: "u-1", email: "ana@x.dev", name: "Ana", role: "professional", mustChangePassword: false };

  const empty = await profileGet(
    makeContext({ db, request: new Request("https://x.test/api/me/profile"), user: ana }) as never,
  );
  assert.equal(empty.status, 200);
  const emptyBody = await empty.json() as { configured: boolean; fallbackDisplayName: string };
  assert.equal(emptyBody.configured, false, "perfil não configurado é declarado, nunca inventado");
  ...
  const saved = await put.json() as { configured: boolean; displayName: string };
  assert.equal(saved.configured, true);
  assert.equal(saved.displayName, "Dra. Ana Souza");
  const persisted = raw.prepare("SELECT display_name FROM user_profiles WHERE user_id='u-1'").get() as { display_name: string };
...
// ── 4. Middleware: rotas públicas e auth opcional ───────────────────────────
{
  ...
  assert.match(middleware, /"\/api\/auth\/signup",/, "signup precisa ser rota pública");
  assert.match(
    middleware,
  ...
  assert.match(signupSource, /SAAS_SIGNUP_ENABLED !== "true"/, "signup permanece fechado por padrão");
  assert.match(signupSource, /registerLoginAbuseFailure/, "tentativas contra e-mails existentes contam no teto de abuso");
}

console.log("✓ self-service SaaS: signup fechado-por-padrão, settings por tenant com RBAC/anti-IDOR, perfil próprio e aceite de convite alcançável");
