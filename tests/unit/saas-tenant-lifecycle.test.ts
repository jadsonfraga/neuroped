...

{
  ...
  assert.equal(
    evaluateEntitlement(pending, "clinical").deniedReason,
    "ENTITLEMENT_NO_SUBSCRIPTION",
  );
...
const lifecycle = fs.readFileSync(
  "functions/api/tenants/[id]/lifecycle.ts",
  "utf8",
);
const exportRoute = fs.readFileSync(
  "functions/api/tenants/[id]/export.ts",
  "utf8",
);
// A coleta do payload migrou para tenant/_exportPayload.ts para que o executor
// assíncrono de LGPD use o MESMO payload. Os invariantes de conteúdo passam a
// ser afirmados lá; o endpoint continua respondendo pelos seus (limite, trilha,
// manifesto).
const exportPayload = fs.readFileSync(
  "functions/api/tenant/_exportPayload.ts",
  "utf8",
);
...
assert.doesNotMatch(
  lifecycle,
  /DELETE\s+FROM/i,
  "endpoint de lifecycle não pode apagar tenant",
);
...
assert.match(exportPayload, /patient-profile:\$\{row\.id\}/);
assert.match(exportPayload, /clinical-event:\$\{row\.id\}/);
assert.match(exportPayload, /TENANT_EXPORT_TOO_LARGE/);
assert.doesNotMatch(exportPayload, /profileEncrypted:/);
assert.doesNotMatch(exportPayload, /payloadEncrypted:/);
...
console.log(
  "saas-tenant-lifecycle: retenção, export íntegro e encerramento seguro aprovados ✔",
);
