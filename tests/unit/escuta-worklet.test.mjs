...
check("captura deriva de amostras reais e drena último bloco",()=>{const {processor:p,messages}=make();p.process([[new Float32Array(24000).fill(.1)]]);p.port.onmessage({data:"stop"});const pcm=messages.find(m=>m.pcm).pcm;assert.equal(pcm.length,8000);assert(pcm.every(v=>Math.abs(v-3277)<=1));const audio=messages.find(m=>m.pcm);assert(Math.abs(audio.peak-.1)<1e-6);assert(Math.abs(audio.rms-.1)<1e-6);assert.equal(messages.at(-1).ended,true);});
check("silêncio digital é reportado sem falso sinal",()=>{const {processor:p,messages}=make();p.process([[new Float32Array(48000)]]);p.port.onmessage({data:"stop"});const audio=messages.find(m=>m.pcm);assert.equal(audio.peak,0);assert.equal(audio.rms,0);});
