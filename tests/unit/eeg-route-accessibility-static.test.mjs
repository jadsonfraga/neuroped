...
const source = await readFile(
  new URL("client/src/pages/eletroencefalograma.tsx", root),
  "utf8",
);
...
test("EEG stays in an accessible internal handoff instead of auto-redirecting", () => {
  ...
  assert.doesNotMatch(source, /href="#\/agendar"/);
  assert.equal(source.match(/href="#\/marcacao"/g)?.length, 2);
  assert.match(source, /sem redirecionamento/);
  assert.ok(app.includes('if (location === "/eletroencefalograma")'));
  assert.match(
    app,
    /<RouteGuard>[\s\S]*?<Route path="\/eletroencefalograma" component=\{EletroencefalogramaPage\}/,
  );
});
