...
const palette = read("client/src/components/CommandPalette.tsx");
...
assert.match(palette, /currentHashPath/, "paleta deve acompanhar a rota atual");
assert.match(palette, /const onPublicRoute = isPublicRoute\(path\)/, "paleta deve reconhecer rota pública no host full");
assert.match(palette, /const privateToolsAllowed = !IS_PUBLIC_ZONE && !onPublicRoute/, "recursos privados devem exigir rota privada");
assert.match(palette, /enabled: patientSearchReady/, "consulta de pacientes deve respeitar o gate privado");
assert.match(palette, /IS_PUBLIC_ZONE \|\| onPublicRoute/, "páginas privadas devem ser filtradas em qualquer superfície pública");
assert.match(palette, /privateToolsAllowed && \(/, "ações e escalas privadas devem depender do gate de rota");
...
console.log("[deep-audit] ✓ fronteiras públicas, stacking, busca privada, fuso da agenda e promoção exata de SHA aprovados.");
