...
import { scales } from "../../client/src/data/scaleFilter.ts";
...
const merged = mergeFilterableCatalog(scales);
const mergedIds = new Set(merged.map((scale) => scale.id));
for (const id of importedIds) assert(mergedIds.has(id), `instrumento não entrou no filtro: ${id}`);

assert(mergedIds.has("abc"), "ABC de Comportamento Aberrante foi removido indevidamente");
assert(mergedIds.has("ica-abc-autismo"), "ICA/ABC-Autismo não foi separado do ABC de Comportamento Aberrante");
