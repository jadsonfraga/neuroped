...
import { verifiedVisualDigestSource } from "./lib/verified-visual-digests.mjs";
...
const publicRoutes = [
  "/familia", "/marcacao", "/eletroencefalograma", "/missao-saude", "/pre-consulta",
  "/pre-retorno", "/efeitos-colaterais", "/verificar", "/portal-familia", "/consentimento-lgpd",
];
...
for (const route of publicRoutes) {
  ...
  const directPublicRoute = new RegExp(`<Route\\s+path="${escaped}"[^>]*component=\\{\\w+\\}[^>]*/>`);
  if (!directPublicRoute.test(app)) {
    fail(`Rota pública deve montar componente direto, sem wrapper clínico: ${route}`);
  }
}

const sourceFiles = walk(root).filter((file) => /\.(ts|tsx|js|jsx|mjs|cjs|ps1|bat|sh|md|json|toml|yml|yaml|txt)$/i.test(file));
const allowDocs = new Set(["SECURITY.md", "docs/CHANGELOG_CIRURGICO.md", "docs/AUDITORIA_CONTINUA_NEUROPED.md"]);
const suspiciousPatterns = [
  { name: "hash SHA-256 fixo de possivel PIN", re: /["'][a-f0-9]{64}["']/i },
  { name: "verificador de PIN hardcoded", re: /\bVITE_PIN_HASH\s*[:=]\s*["']?(?:pbkdf2\$[^\s"']+|[a-f0-9]{64})/i },
  { name: "PIN literal hardcoded", re: /\b(?:MASTER_?PIN|PIN_MASTER|DEFAULT_PIN|HARDCODED_PIN)\b\s*[:=]\s*["'][^"'$\r\n]{4,}["']/i },
  { name: "fallback literal de secret do GitHub", re: /\$\{\{\s*secrets\.[A-Z0-9_]+\s*\|\|\s*["'][^"']+["']\s*\}\}/i },
...
const workflowSecretKeys = new Set([
  "VITE_PIN_HASH", "NEUROPED_MASTER_KEY", "NEUROPED_JWT_SECRET", "JWT_SECRET", "ADMIN_INITIAL_PASSWORD",
  "ADMIN_PW", "ADMIN_MAIL", "CLOUDFLARE_API_TOKEN", "CLOUDFLARE_ACCOUNT_ID", "CF_TOKEN", "CF_ACCOUNT",
  "VERCEL_TOKEN", "VERCEL_ORG_ID", "VERCEL_PROJECT_ID", "CERT_P12_B64", "CERT_P12_PASSWORD", "CERT_B64", "CERT_PW", "RAILWAY_TOKEN",
]);
...
function auditWorkflowSecretAssignments(rel, content) {
  ...
  for (const [index, line] of content.split(/\r?\n/).entries()) {
    ...
    if (!githubSecretReference.test(assignment[2])) {
      fail(`secret/fallback literal em ${rel}:${index + 1} (${assignment[1]} deve referenciar apenas secrets.*)`);
    }
  }
}
...
for (const file of sourceFiles) {
  ...
  try {
    // Not a file allowlist: only proven checksum values are distinguished;
    // every remaining manifest field and all other files still undergo scanning.
    inspected = verifiedVisualDigestSource(rel, content, root);
  } catch (error) {
    fail(`Manifesto visual inválido em ${rel}: ${error instanceof Error ? error.message : String(error)}`);
  }
  for (const pattern of suspiciousPatterns) {
    if (pattern.re.test(inspected)) fail(`${pattern.name} em ${rel}`);
  }
}
