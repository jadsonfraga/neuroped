...
const mediaFiles = [
  ...
  [
    "nesplora-glass-installation.jpeg",
    "nesplora-glass-installation_9e86b34a.jpeg",
  ],
  [
    "nesplora-video-institucional.mp4",
    "nesplora-video-institucional_11252018.mp4",
  ],
...
await cp(path.join(nesploraBuild, "assets"), path.join(destination, "assets"), {
  recursive: true,
});
...
html = html
  ...
  .replace(
    /\s*<script src="\/__manus__\/debug-collector\.js" defer><\/script>/,
    "",
  )
  .replace(
    /\s*<script[^>]+src=["']https:\/\/manus-analytics\.com\/[^>]*><\/script>/gi,
    "",
  )
  .replace(
...
for (const [sourceName, destinationName] of mediaFiles) {
  await copyFile(
    path.join(nesploraAssets, sourceName),
    path.join(mediaDestination, destinationName),
  );
}
...
await writeFile(
  ...
  "# Nesplora estática\n\nEsta pasta é uma cópia autônoma compilada da Nesplora, publicada pelo próprio deploy do NeuroPed em `/nesplora/`. Scripts, estilos, fontes de fallback e arquivos de mídia ficam no próprio pacote; não há dependência de URLs Manus, analytics externos ou fontes remotas em runtime. O `sw.js` local é uma ponte de migração que recupera navegadores com cache legado do NeuroPed. Para atualizar a cópia a partir do projeto-fonte, execute `node scripts/sync-nesplora-static.mjs` no repositório do NeuroPed e revise o diff antes de publicar.\n",
  "utf8",
);

console.log(
  "Nesplora incorporada em client/public/nesplora com ativos locais.",
);
