...
def route_refs(files):
    ...
    for path in files:
        if path.suffix not in {".ts", ".tsx", ".js", ".jsx", ".mjs", ".mts"}:
            continue
        rel = str(path.relative_to(ROOT))
        text = path.read_text("utf-8", errors="replace")
...
def render_findings(title, items, limit=80):
    out = [f"## {title}", "", f"Total: **{len(items)}**", ""]
    if not items:
        return out + ["Nenhum achado.", ""]
    for rel, line, excerpt in items[:limit]:
        safe_excerpt = excerpt.replace("`", "'")
        out.append(f"- `{rel}:{line}` — `{safe_excerpt}`")
    if len(items) > limit:
...
def main():
    ...
    for route in normalized_routes:
        if route in {"/", "*"} or ":" in route:
            continue
        if not any(route_matches(ref, route) for ref in refs):
    ...
    lines += render_findings("Campos TEXT potencialmente sensíveis em migrações SQL sem encrypted/hash", sql_candidates)

    lines += ["## API Cloudflare — inventário", ""]
    lines += [f"- `{api}`" for api in apis]
    lines.append("")

    lines += ["## Sinais de dívida de produto conhecidos", ""]
    for token in ["portal-familia", "portal-acesso", "familia"]:
