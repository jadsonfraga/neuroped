...
- **Já no app:** instrução, comandos, ilustração da aplicadora, câmera, cronômetro e registro.
- **Disponível para preparar em papel:** cenas de apontar/descrever e textos de leitura, extraídos das tarefas vigentes. Impressão individual contém somente imagem ou texto, sem comandos, resposta, botões ou dados da sessão.
...
Durante a coleta, os mesmos recursos podem ser consultados dentro do cartão, mas não há botão de impressão que abra outra janela e encerre a coleta. A tela é da aplicadora, não uma superfície digital nova de testagem infantil. Não foi introduzida equivalência papel/tela. Quando o recurso não foi preparado, documentar a limitação em vez de sair da aba, improvisar ou repetir para completar.
...
Workflow novo `.github/workflows/obs10-guided.yml`: tipos, lint, contrato de recursos das 145 tarefas e jornada de navegador com autenticação e dados sintéticos. Imprime cena/texto isolados, verifica fonte ampliada, teclado, mobile, zoom de texto em 200%, ausência de achados fabricados, proteção de tarefa oral, cópia versus produção livre, livro físico e omissão etária. O workflow OBS-10 original continua obrigatório e não teve asserts removidos; ele percorre todas as 145 tarefas e as regressões de mídia/revisão/exportação.
