...
2. A rotina existente de geração é 19h40/20h20/21h15 Recife (primária/recuperações do mesmo dia), não três novos modelos. O cron de e-mail deste PR é reconciliação 08h07/15h07/19h07, não geração.
3. Resolver a sobreposição #790/#793 antes de mesclar os dois: ambos incluem os mesmos PDFs com fontes e IDs diferentes. Preservar funcionalidades úteis de manifestação/checagem de deploy de #790, mas não a segunda cópia dos instrumentos.
...
5. Fazer merge autorizado com todos os checks obrigatórios; verificar Cloudflare e Vercel no mesmo commit e com os mesmos instrumentos, não apenas HTTP 200. Os PRs abertos não são produção.
