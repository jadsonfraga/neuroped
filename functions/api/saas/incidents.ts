...
export const onRequestGet: PagesFunction<TenantEnv> = async (context) => {
  ...
  const rows = await authorized.db
    .prepare(`SELECT id, component, code, severity, status, correlation_id, metadata_json, created_at FROM saas_incident_events WHERE clinic_id = ? ORDER BY created_at DESC LIMIT 50`)
    .bind(clinicId)
  ...
  return tenantJson({
    ...
    data: (rows.results ?? []).map((row) => ({ id: row.id, clinicId, component: row.component, code: row.code, severity: row.severity, status: row.status, correlationId: row.correlation_id, metadata: safeMetadata(JSON.parse(row.metadata_json || "{}")), createdAt: row.created_at, payloadStored: false })),
  });
};
